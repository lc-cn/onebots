---
"@onebots/adapter-wechat-ilink": minor
"@onebots/core": patch
"@onebots/web": patch
---

## 新增 `@onebots/adapter-wechat-ilink`
...
- **会话持久化**：默认 `{工作目录}/data/wechat-ilink/<account_id>.json`（`JsonFileCredentialStore`），重启恢复登录态。
...
- **`@onebots/core`（patch）**：`Adapter` 的 `id_map` 表名对平台名做安全化，修复平台名含 `-`（如 `wechat-ilink`）时 SQLite 建表失败；`VerificationBlock` 增加 `image_url`，用于远程图片直链展示。
...
- **`@onebots/core`**: sanitize `id_map_*` table names; `VerificationBlock` adds `image_url`.
