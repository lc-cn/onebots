...
> 本文件面向 AI 编码代理（coding agents）。阅读本文后应能在不了解项目背景的情况下正确地进行构建、测试与代码修改。项目文档与代码注释主要使用中文；变量、函数、类型等代码标识符使用英文。

---

## 项目概述

**OneBots** 是一个多平台、多协议的即时通讯（IM）机器人网关与框架（TypeScript / Node.js，pnpm monorepo）。它的定位是「自托管的 IM 机器人中台 + 协议出口」：常驻管理服务负责安装、配置、鉴权和生命周期，独立网关进程接入多个 IM 平台，再以一套或多套开放协议暴露给下游插件 / 业务。网关停止或失败时管理服务仍须可用。
...
- 管理服务与网关基于 Koa（`koa`、`@koa/router`、`koa-body`、`koa-static`），WebSocket 用 `ws`，日志用 `log4js`。
...
│   ├── onebots/       # onebots — 常驻管理服务、独立网关与 CLI（bin 入口 lib/bin.js）
│   │                  #   control/、gateway/、installation/、configuration/ 分别拥有明确职责
...
pnpm dev              # 启动开发工作区的管理服务；网关由管理服务按已激活运行版本托管
...
- 扩展通过 Web/TUI 或 `onebots control plan/install/installation/activate` 进入不可变候选运行版本；公开 CLI 不再接受 `-r/-p/-t/-c` 直接加载网关插件。新增安装入口必须复用同一计划、必需 peer、验证和显式激活流程。
- 管理服务默认端口 **6727** 并持有公共入口；网关只监听私有地址。运行时配置文件为 `config.yaml`（`general` 写协议默认值，`{platform}.{account_id}` 下写账号与平台密钥），但只有管理服务可以提交和应用配置版本。
...
- **真实账号集成测试需要先启动管理服务并通过控制入口启动网关**，再通过环境变量指定测试账号：
...
- **永远不要把含真实凭据的 `config.yaml`、token、密钥提交进仓库**；`data/` 是本地运行时目录。协议实例的 `access_token` 只用于对应协议鉴权；Web 管理认证使用设备码配对和可撤销会话，与网关配置隔离。
