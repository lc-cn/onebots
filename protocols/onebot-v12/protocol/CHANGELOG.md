...
- 3dfda6c: 拆分 OneBot V12 动作与传输边界，完整清理 Webhook 监听器，并允许适配器显式承接协议可选动作。
...
- 9124055: 实现 OneBot 12 的频道改名、频道成员详情与频道成员列表动作，并通过 canonical Adapter 接口保留各平台实现能力；未建模的 Guild 动作回落到显式平台能力路由。
...
- 894d80a: 将反向 WebSocket 的连接、无限重连与停止生命周期封装为可释放会话，修复协议停止后旧连接仍会恢复的问题；同时在 connect 元事件中等待真实版本信息，避免把未解析 Promise 序列化为空对象。
...
- 10e0dd5: 让 `get_supported_actions` 根据当前适配器能力清单动态投影 OneBot 12 标准动作与平台扩展动作，修复能力误报、漏报及尚未实现的文件动作提前暴露问题。
