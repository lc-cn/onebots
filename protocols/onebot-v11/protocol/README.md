...
启动管理服务并完成设备配对后，在 Web 控制台或 TUI 中选择 OneBot v11 协议和所需平台适配器。确认安装计划，等待依赖安装和校验完成，再显式激活候选运行版本。
...
协议扩展激活后，为账号填写 `onebot.v11` 配置并应用，再通过管理服务启动网关。协议未安装或未激活时，仅填写配置不会加载协议。
