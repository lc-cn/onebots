...
- ea72bbf: 补齐微信客服知识库分组与问答的完整原生动作，并统一按事件实际 `open_kfid` 投影 Bot 身份、发现多客服账号状态及闭合登录身份。
- f22eab6: 抽取与对象键顺序无关的 canonical JSON 指纹原语，统一多个适配器的事件回退身份；修复飞书事件缺少 `event_id` 时依赖接收时钟、导致重试事件获得不同 ID 的问题。
- 620c51b: 让微信客服首次凭证请求、补偿同步与协议出口共享账号启动取消边界，避免超时或热重载后旧网络任务继续运行。
...
- ee188c2: 闭合微信客服命名动作参数，校验批量客户上限与接待部门 ID，并隔离复杂请求对象。
...
- 99be7ef: 修复微信客服异步协议投递早于去重与游标提交的问题，并补齐 Fetch/Koa Host 与可靠生命周期接入。
- 91f0a7f: 让七个适配器复用 core 的包版本读取模块，删除各自重复的文件读取、JSON 解析与错误处理实现。
- d8fd796: 校准微信客服事件内层身份投影，阻止同步凭证与回调明密文进入下游事件；为官方 API 响应增加运行时结构校验和 JSON/二进制闭合重载，并隔离业务监听器异常、统一原生动作注册表。
...
- bf87da0: 修正微信客服同步与回调在业务投递失败后仍确认事件的问题，并串行化多客服账号的原子游标提交。
...
- 0f38afc: 增加微信客服 manual 接入模式、核心结构化错误、运行时配置闭合与动态 Web 表单，并收紧 Webhook 路径约束。
