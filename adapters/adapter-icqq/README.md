...
| 交互按钮         |  ✅  |  ✅  | 完整按钮布局、权限与点击行为              |
| 合并转发         |  ✅  |  ✅  | 已有资源与自定义转发节点                  |
| 原生引用         |  ✅  |  ✅  | 完整 seq/rand 与被引用消息                |
| 文件             |  ✅  |  ✅  | 原生文件段或标准 `upload_file`            |
| 频道论坛         |  -   |  ✅  | 保留论坛帖子标识与创建时间                |
| JSON             |  ✅  |  ✅  | JSON 卡片                                 |
| XML              |  ✅  |  ✅  | XML 卡片                                  |
...
图片、闪照、语音、视频和泡泡消息会保留 ICQQ/QQNT 的缓存、超时、请求头、转码、临时文件、媒体哈希、尺寸、时长与 NT 文件标记。结构化按钮和 Markdown 签名配置在进入 SDK 前会完整校验，错误字段不会被静默忽略。
...
| 分类    | 已支持能力                                                                                            |
| ------- | ----------------------------------------------------------------------------------------------------- |
| 消息    | 私聊、群聊与 QQ 频道发送/撤回，匿名群消息、系统申请、消息获取、历史记录、合并转发制作与读取、标记已读 |
...
| 文件    | 私聊/群文件上传与删除、群目录分页与管理、移动与重命名、完整下载元数据及下载地址                       |
...
账号停止会先完成客户端解绑与本地状态清理；若平台登出失败，错误同时通过 `stop_error` 报告并向账号生命周期传播，不会把失败伪装成正常关闭。
