...
| 好友与资源 | 好友分组管理、漫游表情、陌生人列表、图片 OCR、视频地址            |
| 群与频道   | 群签到、群分享、成员消息屏蔽、从群添加好友、频道帖子分享地址      |
| 消息       | 群临时消息、讨论组消息                                            |
| 缓存同步   | 重载好友、陌生人、群、频道和黑名单                                |

扩展动作的参数名沿用 ICQQ 语义，并在调用底层客户端前严格验证类型。可通过 `get_supported_actions` 获取当前版本的完整动作名，避免依赖文档中的静态清单。
