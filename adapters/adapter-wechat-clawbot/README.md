...
| `get_context_token` | `user_id`                                  | 读取当前对端的回复上下文；尚无上下文时返回 `null`        |

最近媒体缓存保留 256 条事件；长期保存应在收到事件后及时下载。命名动作会拒绝未约定的顶层参数，避免拼写错误被静默忽略。登录信息与事件 `bot_id` 始终使用会话中的真实 `ilink_bot_id`，OneBots `account_id` 只作为内部配置键。
