...
export class InstagramHttpHost {
    ...
    constructor(
        ...
        private readonly resolveClient: (accountId: string) => InstagramClient | undefined,
    ) {}
    ...
    mount(accountId: string, client: InstagramClient): void {
        ...
        if (!path) throw InstagramError.invalid("webhook 模式必须配置 http_path");
    ...
    private async accept(path: string, ctx: InstagramKoaContext): Promise<void> {
