...
- 平台扩展：公开常用 Guild、频道、角色、线程、邀请、Reaction、Auto Moderation、Scheduled Event、Guild Emoji、Soundboard、消息搜索和语音频道状态动作，并保留受约束的完整 Discord v10 REST 入口。
...
- 搜索：`search_guild_messages`，支持 `channel_id`、`author_id`、`mentions` 等官方重复 query 参数；需要 `READ_MESSAGE_HISTORY` 权限与 `MESSAGE_CONTENT` Intent
...
- 语音与 Soundboard：`set_voice_channel_status`、`send_soundboard_sound`、`list_default_soundboard_sounds`，以及 Guild Soundboard Sound 查询、创建、更新和删除动作
