...
- 7d44567: 完善钉钉 Stream、Webhook 与 manual 接入的投递确认和成功去重语义，使用真实平台机器人身份，补齐通讯录与场景群原生动作，并统一能力声明和可选参数校验。
- 9b5f106: 统一钉钉配置、消息、回调、网络与开放平台错误，导出类型化 Bot 事件，并修复并发启动、停止竞态及 Stream 首次连接失败后无法重新创建连接的问题。
...
- ec8ad3b: 让钉钉账号启动响应 OneBots 生命周期取消信号，中止访问令牌请求、关闭未完成的 Stream 连接，并修正事件接入模式文档。
- dbe0663: 统一钉钉结构化 HTTP 接入边界，支持 Fetch/Koa 已有 Host，并保证全部事件出口尝试完成后再确认回调。
...
- f64968e: 递归读取应用可见的钉钉组织通讯录，完整分页场景群成员并校验成员详情，同时合并并发访问令牌刷新。
- 3c06d43: 补齐钉钉互动卡片实例创建、投放、整体更新与 AI 流式更新动作，并将卡片交互回调投影为带实例、用户、场域和动作数据的结构化事件上下文。
- f48058a: 增加不创建 Stream 或 Webhook 路由的 manual 接收模式，并公开统一的钉钉原始事件 ingest 入口。
- e246153: 重构钉钉适配器的 Stream、HTTP 回调与发送模式，补齐加密验证、真实事件投影、企业机器人富消息、通讯录和开放平台扩展动作，并修复群 ID 被错误当作用户 ID 发送的问题。
...
- 68c600e: 使钉钉 Stream 断开失败不再跳过状态清理和异步停止监听器，并传播结构化停止错误。
...
- 7665728: 严格校验钉钉消息段、远程资源和混合消息，正确还原统一用户 ID 的 @，避免未知段与无效内容静默退化。
- ea8074e: 补齐钉钉企业机器人消息撤回与状态查询，逐用户投影批量成员事件，并统一收紧 OpenAPI 路径边界。
