...
已有 HTTP Host、消息队列或测试连接可以配置 `receive_mode: manual`，无需另开端口：

- 已经完成验签和解密的载荷调用 `await bot.ingest(rawEvent)`；
- Node Host 调用 `ingestHttp({ method, query, body })`，获得 `{ status, headers, body, event? }`；
- Fetch/WinterCG Host 直接调用 `await bot.acceptHttp(request)`；Koa Host 可调用 `await bot.acceptHttp(ctx)`。

三个入口共用验签、解密、身份发现、并发合并、去重与事件投影链路。使用 `ingestHttp()` 或 `acceptHttp()` 时仍需配置 `token`、`encrypt_key` 与 `corp_id`；只把已验证载荷交给 `ingest()` 时可省略。Promise 成功表示全部 canonical 事件监听器都已尝试完成；任一出口失败会返回 500 且不会提交去重，钉钉可安全重投。
