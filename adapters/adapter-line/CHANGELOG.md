...
- e1f2306: 统一 LINE manual、Webhook 与原始事件接入边界，增加 typed 事件、结构化错误、动态配置表单及更严格的请求归属和 URL 校验。
- 167965d: 统一 LINE Fetch/Koa Webhook 的跨 realm 结构化 HTTP 边界与错误上报依赖，修正 manual 模式验签字段展示，并完善类型、文档与回归测试。
- 9a09197: 统一由 LINE 领域动作注册表生成能力发现，并严格校验原生动作的可选参数，避免错误输入被静默忽略。
- 66107f6: 将 LINE 会员、账号绑定、Beacon、视频播放完成与电话通知送达事件投影为可过滤的精确通用通知，并为 Module 与 Bot 生命周期事件补充稳定子类型和原生扩展载荷。
- 18782ea: 修正 LINE 未知消息段被静默丢弃、SDK 版本报告不真实和首次身份探针失败后永久离线的问题；启动探针现在会无限退避恢复，并在账号停止时取消。
...
- dd88d57: 接通 LINE 标准已读与按消息 ID 回复能力，持久化官方消息 token，并保留已缓存的会话名称。
- 62e0f3e: 迁移至官方 LINE SDK 11.x，补齐安全 Webhook 验签与去重、完整事件投影、原生消息段、聊天上下文持久化、群组与聊天室、Rich Menu、会员、洞察、内容下载及结构化错误能力，并更新配置 Schema、文档和契约测试。
- cfafb73: 按 LINE 官方身份与会话生命周期重构事件投影：使用 destination / Bot Info user ID 作为 bot_id，补齐机器人加入和离开 canonical notice，将批量成员事件逐人投影，并在离开后清理持久化会话目录；按领域补齐 Audience、LIFF、Module / Chat Control、Mission Sticker 及群聊身份原生动作。
...
- 4cb4f09: 按始终可用、会话上下文与专项权限细分 LINE 原生动作能力提示，并将 Audience 动作统一接入精确参数包装器。
...
- 417e68e: 将 LINE 的 72 个原生动作按消息、Rich Menu 和统计领域拆分，并由单一不可变注册表统一派生能力发现与执行。
- 228dce6: 为 LINE 关注者与群成员目录增加去重、游标推进校验和受控资料并发，并在编辑与撤回事件中保留聊天上下文。
- 67bdebd: 统一九个适配器的严格 TypeScript 契约，并收紧 Discord 成员、Slack 消息以及 LINE、Teams、Telegram 可空数据的运行时边界。
- 5fdf540: 闭合 LINE 群组、LIFF、Module、Mission Sticker、会员与洞察动作参数，校验官方请求字段、枚举、分页、TTL 和统计日期范围。
- e54835e: 为 LINE Rich Menu 与 Coupon 动作增加共用精确参数入口，严格校验图片格式和大小、alias、批量用户与分页参数，并拒绝未知外层字段。
- 17c5655: 闭合 LINE 消息、成员、投递统计与 Webhook 动作参数，校验官方批量、日期、重试键、loading 与分页边界，并为全量成员分页增加循环保护。
- 95a5bf1: 闭合 LINE Audience 平台动作的 JSON、文件、分页与枚举参数契约，拒绝未知字段、非法 ID、越界分页和伪 Base64，并支持当前官方受众来源筛选值。
- c87ba0a: 补齐 LINE Channel Access Token 的短期、stateless、v2.1/JWT 签发、校验、key ID 查询和撤销能力，并隔离配置凭据与动作参数。
