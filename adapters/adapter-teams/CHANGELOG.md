...
- edc1164: 等待 Teams 的异步 ready 与 stopped 生命周期监听器完成，保证宿主启动和退出编排具有确定语义。
- 2625604: 拆分 Teams Conversation、OAuth 与 Graph 动作领域，补齐回复和 targeted Activity 生命周期，统一能力发现并严格校验可选参数。
- c82c4b7: 让 Teams HTTP、manual 与既有 Agents SDK 入口共享可等待、可重试且并发去重的 Activity 投递管线，并为缺失 Activity ID 的事件生成稳定身份。
...
- 4c5732f: 阻止 Microsoft Teams 账号在启动超时或停止后被迟到完成的生命周期任务重新标记为在线。
- 85784a8: 收紧 npm 发布边界：TypeScript 包不再携带测试产物，Web 管理端只发布构建后的 `dist`。发布流水线会真实打包全部工作区包，拒绝测试、源码、`node_modules` 泄漏或缺失入口的 tarball。
- cbd6eb5: 统一 Teams 的 Activity 入站、Graph 客户端、生命周期与结构化错误边界，补齐重试去重、逐项 Reaction 投影及安全配置 Schema。
...
- 8c7a844: 统一校验 Microsoft Graph、Entra Authority 与 Bot Connector 的 HTTPS 配置边界，并修正 Koa 响应桥在 `end()` 后的 WebResponse 状态，避免 SDK 重复写入已结束响应。
- 4af3af9: 严格校验 Teams 消息段和 HTTPS 媒体附件，支持完整 file consent 上传、标准文件信息卡片与接收回复投影，并修正未知段静默丢失及 SDK 版本报告不真实的问题。
- df95394: 增加不注册 OneBots 路由的 manual 接入模式，并提供宿主无关的 ingestHttp 结构化响应与 acceptHttp 入口。
- 74e5404: 迁移至 Microsoft 365 Agents SDK，持久化真实 ConversationReference，补齐 Teams 富消息、完整 Activity 投影、团队/频道/成员、反应、会议、文件卡片、主动会话与 Graph 扩展动作，并加入严格 JWT、serviceUrl allowlist、结构化错误、动态 Schema、文档和契约测试。
- a950886: 统一保留频道事件的服务器与频道双层地址，修正 OneBot V12 和 Satori 的频道事件投影，并让 OneBot V12 发送使用独立的 `guild_id` 与 `channel_id`。
- 38b9192: 严格区分 Teams groupChat 与团队频道，canonical Group 不再混入 channel 会话，频道继续通过保留真实上下文的原生动作访问。
...
- 59643af: 将 Teams file consent 上传绑定到已认证的 accept Invoke Activity：`complete_file_consent_upload` 改用 `consent_activity_id`，不再接受调用方指定上传 URL、会话和 file-info 元数据，并支持并发合并及上传后回执重试。
- 67bdebd: 统一九个适配器的严格 TypeScript 契约，并收紧 Discord 成员、Slack 消息以及 LINE、Teams、Telegram 可空数据的运行时边界。
- 7e22335: 扩展 Microsoft Teams 原生 Activity：双向保留 AI 标签、引用、敏感度、反馈回路与建议操作，增加受会话边界保护的原生 Activity 发送动作并支持流式中间响应。
- 3967bb7: 补齐 Microsoft Teams 引用回复的双向消息段和个人聊天 Read Receipt 标准事件，并升级至当前稳定 Teams API 2.0.15。
...
- 6e584a8: 让 Microsoft Teams 从单一不可变注册表派生平台动作发现与执行，并按会话边界拆分原生动作处理器。
