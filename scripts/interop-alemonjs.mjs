...
const RUNTIME = path.resolve(process.env.ONEBOTS_INTEROP_ALEMONJS_RUNTIME ?? "interop/alemonjs");
const TOKEN = "onebots-alemonjs-interop-token";
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "onebots-alemonjs-interop-"));
...
try {
    ...
    const alemonjs = startProcess(
        process.execPath,
        [path.join(ROOT, "interop/alemonjs/app.mjs")],
        {
            ONEBOTS_INTEROP_ALEMONJS_RUNTIME: RUNTIME,
    ...
    children.push(alemonjs);
    const evidence = await waitForEvidence(evidencePath, [gateway, alemonjs], 20_000);
    assertEvidence(evidence);
    process.stdout.write(
        `${JSON.stringify({ ok: true, framework: "alemonjs", frameworkVersion: evidence.frameworkVersion, adapterVersion: evidence.adapterVersion, protocol: "onebot.v11", transport: "websocket", checks: ["auth-rejection", "handshake", "private-message", "get_login_info", "send_private_msg"] })}\n`,
    );
} finally {
...
function assertRuntime() {
    for (const file of [
        "node_modules/alemonjs/package.json",
        "node_modules/@alemonjs/onebot/package.json",
    ]) {
        if (!fs.existsSync(path.join(RUNTIME, file)))
            throw new Error(`AlemonJS 互操作依赖缺失：${file}`);
    }
}
...
function assertEvidence(evidence) {
    const failures = [];
    if (evidence.framework !== "alemonjs") failures.push("framework");
    if (evidence.frameworkVersion !== "2.1.103") failures.push("frameworkVersion");
    if (evidence.adapterVersion !== "2.1.21") failures.push("adapterVersion");
    if (evidence.connection?.activeVersion !== 11 || evidence.connection?.state !== "ready")
    ...
    if (failures.length)
        throw new Error(
            `AlemonJS 互操作证据不完整：${failures.join(", ")}\n${JSON.stringify(evidence)}`,
        );
}
