...
const TOKEN = "onebots-zerobot-interop-token";
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "onebots-zerobot-interop-"));
...
try {
    ...
    const zerobot = startProcess(
        ...
        "ZeroBot",
        path.join(ROOT, "interop/zerobot"),
    );
    children.push(zerobot);
    const evidence = await waitForEvidence(evidencePath, [gateway, zerobot], 60_000);
    assertEvidence(evidence);
    process.stdout.write(
        `${JSON.stringify({ ok: true, framework: "zerobot", frameworkVersion: "1.8.2", adapterVersion: "built-in", protocol: "onebot.v11", transport: "websocket", checks: ["auth-rejection", "handshake", "private-message", "get_login_info", "send_private_msg"] })}\n`,
    );
} finally {
...
function assertEvidence(evidence) {
    const failures = [];
    if (evidence.framework !== "zerobot") failures.push("framework");
    if (evidence.frameworkVersion !== "1.8.2") failures.push("frameworkVersion");
    ...
    if (!evidence.messageId || evidence.messageId === "0") failures.push("messageId");
    if (failures.length)
        throw new Error(
            `ZeroBot 互操作证据不完整：${failures.join(", ")}\n${JSON.stringify(evidence)}`,
        );
}
