...
const TOKEN = "onebots-astrbot-interop-token";
const PYTHON =
    process.env.ONEBOTS_INTEROP_ASTRBOT_PYTHON ??
    path.join(ROOT, "interop/astrbot/.venv/bin/python");
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "onebots-astrbot-interop-"));
...
try {
    ...
    const astrbot = startProcess(
        PYTHON,
        [path.join(ROOT, "interop/astrbot/app.py")],
        {
        ...
        "AstrBot",
        path.join(ROOT, "interop/astrbot"),
    );
    children.push(astrbot);
    await waitForPort(frameworkPort, astrbot, 20_000);
    ...
    const evidence = await waitForEvidence(evidencePath, [astrbot, gateway], 30_000);
    assertEvidence(evidence);
    process.stdout.write(
        `${JSON.stringify({ ok: true, framework: "astrbot", frameworkVersion: "4.28.0b1", adapterVersion: "1.4.4", protocol: "onebot.v11", transport: "reverse-websocket", checks: ["auth-rejection", "handshake", "private-message", "get_login_info", "send_private_msg"] })}\n`,
    );
} finally {
...
async function assertPythonDependencies() {
    if (!fs.existsSync(PYTHON)) {
        throw new Error(
            "AstrBot 互操作虚拟环境缺失，请按 requirements-source.txt 与 requirements.txt 创建 .venv",
        );
    }
    ...
    if ((await probe.exit) !== 0) throw new Error(`AstrBot 互操作依赖版本不符\n${probe.logs()}`);
}
...
function assertWrongTokenRejected(port) {
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => reject(new Error("AstrBot 未拒绝错误 token")), 3_000);
        socket.once("open", () => reject(new Error("AstrBot 接受了错误 token")));
...
function assertEvidence(evidence) {
    const failures = [];
    if (evidence.framework !== "astrbot") failures.push("framework");
    if (evidence.frameworkVersion !== "4.28.0b1") failures.push("frameworkVersion");
    if (evidence.adapterVersion !== "1.4.4") failures.push("adapterVersion");
    ...
    if (failures.length)
        throw new Error(
            `AstrBot 互操作证据不完整：${failures.join(", ")}\n${JSON.stringify(evidence)}`,
        );
}
