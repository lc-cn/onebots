...
const TOKEN = "onebots-kovi-interop-token";
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "onebots-kovi-interop-"));
...
try {
    ...
    const kovi = startProcess(
        "cargo",
        ["run", "--locked", "--quiet"],
        {
            ONEBOTS_INTEROP_HOST: "127.0.0.1",
            ONEBOTS_INTEROP_PORT: String(gatewayPort),
            ONEBOTS_INTEROP_PATH: "/mock/interop/onebot/v11",
            ONEBOTS_INTEROP_TOKEN: TOKEN,
            ONEBOTS_INTEROP_EVIDENCE: evidencePath,
        },
        "Kovi",
        path.join(ROOT, "interop/kovi"),
    );
    children.push(kovi);
    const evidence = await waitForEvidence(evidencePath, [gateway, kovi], 120_000);
    assertEvidence(evidence);
    process.stdout.write(
        `${JSON.stringify({ ok: true, framework: "kovi", frameworkVersion: "0.13.0", adapterVersion: "0.13.2", protocol: "onebot.v11", transport: "websocket-split", checks: ["auth-rejection", "handshake", "private-message", "get_login_info", "send_private_msg"] })}\n`,
    );
} finally {
...
function assertWrongTokenRejected(port) {
    return new Promise((resolve, reject) => {
        const socket = new WebSocket(
            `ws://127.0.0.1:${port}/mock/interop/onebot/v11/api?access_token=wrong`,
        );
...
function assertEvidence(evidence) {
    const failures = [];
    if (evidence.framework !== "kovi") failures.push("framework");
    if (evidence.frameworkVersion !== "0.13.0") failures.push("frameworkVersion");
    if (evidence.adapterVersion !== "0.13.2") failures.push("adapterVersion");
