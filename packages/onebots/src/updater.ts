...
export function packageNamesFor(adapters: string[], protocols: string[]): string[] {
    return ["onebots", ...adapters.map(name => `@onebots/adapter-${name}`), ...protocols.map(name => `@onebots/protocol-${name}`)];
}
...
export async function runUpdate(options: UpdateOptions): Promise<void> {
    ...
    const updates = packages.map(name => ({ name, current: installedVersion(name, runtimeRoot), latest: latestVersion(manager, name) }));
    for (const item of updates) console.log(`${item.name}: ${item.current ?? "未安装"} -> ${item.latest ?? "无法查询"}`);
    const unavailable = updates.filter(item => !item.latest);
    if (unavailable.length) throw new Error(`无法查询包版本: ${unavailable.map(item => item.name).join(", ")}`);
    const changed = updates.filter(item => item.current !== item.latest);
    if (!changed.length) return void console.log("已是最新稳定版本");
    ...
    if (!options.yes) {
        ...
        try {
            const answer = (await prompt.question(`更新 ${changed.length} 个包并修改锁文件？ [y/N] `)).trim().toLowerCase();
            if (answer !== "y" && answer !== "yes") return void console.log("已取消");
        } finally { prompt.close(); }
    }

    if (isEphemeralNpx()) {
        const command = manager === "pnpm" ? `pnpm add ${changed.map(item => `${item.name}@latest`).join(" ")}` : `npm install ${changed.map(item => `${item.name}@latest`).join(" ")}`;
    ...
    if (manager === "pnpm") {
        execFileSync("pnpm", projectRoot ? ["up", ...names] : ["add", "--global", ...names], { cwd: projectRoot ?? runtimeRoot, stdio: "inherit" });
    } else {
        execFileSync("npm", ["install", ...(projectRoot ? [] : ["--global"]), ...names], { cwd: projectRoot ?? runtimeRoot, stdio: "inherit" });
    }
    if (spec) {
        const wasRunning = controller.status().running;
        await controller.install({ ...spec, nodePath: process.execPath, binPath: path.resolve(process.argv[1]) });
        if (wasRunning) {
            if (options.yes || await confirmRestart()) await controller.restart();
        }
    }
    console.log("OneBots 及插件更新完成");
}
...
function latestVersion(manager: "npm" | "pnpm", name: string): string | null {
    try { return execFileSync(manager, ["view", name, "version"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }).trim(); }
    catch { return null; }
}
...
function findProjectRoot(from: string): string | null {
    ...
    while (current !== path.dirname(current)) {
        ...
        if (fs.existsSync(manifest)) {
            ...
            if (parsed.dependencies?.onebots || parsed.devDependencies?.onebots || parsed.dependencies?.["@onebots/core"]) return current;
        }
        current = path.dirname(current);
    }
    return null;
...
async function confirmRestart(): Promise<boolean> {
    const prompt = readline.createInterface({ input: process.stdin, output: process.stdout });
    try {
        const answer = (await prompt.question("服务正在运行，立即重启？ [y/N] ")).trim().toLowerCase();
