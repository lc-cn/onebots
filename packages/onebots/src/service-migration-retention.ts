...
export async function retainServiceMigrationRuntime(
    ...
    if (source !== original.workingDirectory || !within(source, original.binPath))
    ...
    const runtime = await captureLegacyRuntimeTree(
        source,
        path.join(store, "programs"),
        id,
        legacyRuntimeExclusions(original, source),
    );
