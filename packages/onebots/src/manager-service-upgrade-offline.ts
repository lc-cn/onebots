...
async function inspectStoppedManagerServiceUpgrade(
    ...
    if (record.action !== "upgrade" || !record.upgrade || (complete ? record.phase !== "releasing" : !["releasing", "completed"].includes(record.phase)) ||
        record.upgrade.snapshot.initial.processId !== null ||
        record.upgrade.snapshot.platform !== host.platform ||
        !(complete ? ["running", "interrupted"] : ["running", "interrupted", "succeeded"]).includes(record.status)) throw failure();
    ...
    try {
        ...
        try {
            ...
            if (!marker || (!complete && marker.phase !== "released") || marker.operationId !== record.id ||
...
export function releaseStoppedManagerServiceUpgrade(
    record: ManagerServiceRecord, host: ServiceHost, platform?: ServicePlatform,
): Promise<void> {
