            ...
            client.on("message", async raw => {
                ...
                switch (payload.action) {
                    ...
                    case "system.saveConfig":
                        ...
                        writeConfigFileAtomic(BaseApp.configPath, String(payload.data), {
                            backup: true,
                        });
...
export function createOnebots(config: BaseApp.Config | string = "config.yaml") {
    ...
    if ((!config.username || !config.password) && !hasAccessToken) {
        ...
        writeCliError("[onebots] 已自动生成管理端账号并写入配置文件，请尽快在 Web 端修改密码：");
        writeCliError(`  用户名: ${generatedUser}`);
        writeCliError(`  密码: ${generatedPass}`);
