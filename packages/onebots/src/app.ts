...
const client = (() => {
    ...
    for (const dir of candidates) {
        console.debug('[onebots] 查找 Web 前端目录:', dir);
        if (existsSync(dir)) {
            console.info('[onebots] 使用 Web 前端目录:', dir);
            return dir;
        }
    }
    console.warn('[onebots] 未找到 @onebots/web/dist，管理端页面将不可用');
    return '';
})();
...
export class App extends BaseApp {
    ...
    get logClients() { return this._logCache.clients; }
    get logCacheFile() { return this._logCache.cacheFile; }
    get verificationClients() { return this._verification.clients; }
    get pendingVerifications() { return this._verification.pending; }
    get messageDebug() { return this._messageDebug; }
    ...
    protected onAdapterCreated(adapter: import("@onebots/core").Adapter): void {
        ...
        adapter.on('message:dispatch', (payload: { platform: string; account_id: string; event: unknown }) => {
            this._messageDebug.recordInbound(payload.platform, payload.account_id, payload.event);
        });
        adapter.on('message:protocol-dispatch', (payload: { platform: string; account_id: string; protocol: string; version: string; data: unknown }) => {
            this._messageDebug.recordOutbound(payload.platform, payload.account_id, payload.protocol, payload.version, payload.data);
        });
    }
            ...
            client.on("message", async raw => {
                ...
                switch (payload.action) {
                    case "system.input":
                        ...
                        process.nextTick(() => process.stdin.emit("data", Buffer.from(payload.data + "\n", "utf8")));
...
export function createOnebots(
    ...
    if (!existsSync(BaseApp.configPath) && isStartWithConfigFile) {
        ...
        console.info("[onebots] 已创建默认配置文件:", BaseApp.configPath);
    }
    if (!isStartWithConfigFile) {
        writeFileSync(BaseApp.configPath, yaml.dump(config));
        console.info("[onebots] 已自动保存配置到:", BaseApp.configPath);
    }
    if (!existsSync(BaseApp.dataDir)) {
        mkdirSync(BaseApp.dataDir);
        console.info("[onebots] 已创建数据存储目录:", BaseApp.dataDir);
    }
    ...
    if ((!config.username || !config.password) && !hasAccessToken) {
        ...
        console.warn("[onebots] 已自动生成管理端账号并写入配置文件，请尽快在 Web 端修改密码：");
        console.warn("  用户名:", generatedUser);
        console.warn("  密码:", generatedPass);
        console.warn("  配置文件:", BaseApp.configPath);
    }
    configure({
        appenders: {
            out: {
