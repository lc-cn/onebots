...
describe("configuration route", () => {
    it("后创建实例切换静态默认目录后仍读取并公开自己的运行路径", () => {
        const { app, configHandler, systemHandler } = setup(
        ...
        const otherDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "onebots-config-other-"));
        directories.push(otherDirectory);
        BaseApp.configDir = otherDirectory;

        const configContext = { set: vi.fn() } as unknown as RouterContext;
        const systemContext = { set: vi.fn() } as unknown as RouterContext;
        configHandler(configContext);
        systemHandler(systemContext);

        expect(configContext.body).toBe("access_token: old-token\n");
        expect(systemContext.body).toMatchObject(expected);
    });
    ...
    it("返回已保存并生效的机器可读状态", async () => {
        ...
        expect(ctx.set).toHaveBeenCalledWith(
            "X-OneBots-Config-Revision",
            createManagementConfigRevision("access_token: next-token\n"),
        );
    ...
    it("宿主配置保留文件并返回重启字段", async () => {
        ...
        const ctx = configMutationContext("port: 7000\n");
    ...
    it("无效 YAML 返回 400 且不写入文件", async () => {
        const reload = vi.fn(async () => undefined) as App["reload"];
        const { handler } = setup(reload);
        const ctx = configMutationContext("accounts: [\n");
...
function configMutationContext(body: unknown): RouterContext {
    return {
        get: () => "",
        set: vi.fn(),
        request: { body },
