...
export function registerConfigRoutes(app: App, router: Router): void {
    ...
    router.post("/api/config", async (ctx: RouterContext) => {
        ...
        try {
            const result = await saveAndApplyRuntimeConfig(app, configContent);
            ...
            ctx.body = result.restartRequired
                ? {
                      success: true,
                      ...result,
                      message: `配置已保存；${result.changedHostFields.join(", ")} 需要重启后生效`,
                  }
                : { success: true, ...result, message: "配置已保存并生效" };
        } catch (error) {
            ctx.status =
