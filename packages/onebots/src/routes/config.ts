import { RouterContext, BaseApp, writeConfigFileAtomic } from "@onebots/core";
...
export function registerConfigRoutes(app: App, router: Router): void {
    ...
    router.post("/api/config", async (ctx: RouterContext) => {
        ...
        try {
            validateRuntimeConfig(parseRuntimeConfig(configContent));
        } catch (error) {
            ctx.status = 400;
        ...
        try {
            writeConfigFileAtomic(BaseApp.configPath, configContent, { backup: true });
