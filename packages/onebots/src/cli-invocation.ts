...
const RUNTIME_OPTIONS = new Set(["-c", "--config", "-r", "--register", "-p", "--protocol"]);
...
export function prepareCliInvocation(argv: string[]): CliInvocation {
    ...
    if (!firstPositional) {
        ...
        const rootOnly = trailing.length > 0 && trailing.every(token => ["-h", "--help", "-v", "--version"].includes(token));
        return rootOnly ? { kind: "cli", argv } : { kind: "cli", argv: [argv[0], argv[1], "run", ...argv.slice(2)] };
    }
    if (REMOVED_NAMESPACES.has(firstPositional.token)) return { kind: "unknown", command: firstPositional.token };
    ...
    return {
        kind: "cli",
        argv: [argv[0], argv[1], ...remaining, argv[routeIndex], ...local.remaining, ...runtime, ...local.runtime],
    };
}
...
function findInvalidRuntimeOption(tokens: string[]): string | undefined {
    for (let index = 0; index < tokens.length; index++) {
        ...
        } else if (/^--(?:config|register|protocol)=$/u.test(token)) {
...
function isAttachedRuntimeOption(token: string): boolean {
    return /^(?:--(?:config|register|protocol)=|-[crp].+)/u.test(token);
}
