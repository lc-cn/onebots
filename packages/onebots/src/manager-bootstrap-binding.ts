...
export function managerBootstrapBindingDirectory(home: string, id: string): string {
    ...
    try {
        ...
        if ((error as NodeJS.ErrnoException).code === "ENOENT") {
            if (readManagerMigrationOrigin(path.dirname(home))) return cycleDirectory(home, id);
            if (
                fs.existsSync(path.join(home, "operations")) ||
                fs.existsSync(path.join(home, "bootstrap-cycles"))
            )
    ...
    const intent = closedServiceObject(storage.read("intent.json"), [
        "schemaVersion",
        "id",
        "service",
        "planDigest",
    ...
    return cycleDirectory(home, id);
}
function cycleDirectory(home: string, id: string): string {
