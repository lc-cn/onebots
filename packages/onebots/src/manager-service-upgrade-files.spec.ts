...
function fixture(platform: "linux" | "darwin" = "linux") {
    const root = fs.realpathSync(fs.mkdtempSync("/tmp/ob-upgrade-files-")); roots.push(root);
    const host: ServiceHost = { platform, homedir: root, uid: process.getuid?.(), env: {},
        exec: vi.fn(() => { throw new Error("must not call OS"); }),
        spawn: vi.fn(async () => { throw new Error("must not spawn"); }) };
    const previous: ManagerServiceSpec = { schemaVersion: 1, runtimeKind: "control", scope: "user",
        workspace: path.join(root, "data"), workingDirectory: root, binPath: path.join(root, "old.js"),
    ...
    const snapshot = structuredClone(captured.snapshot); captured.dispose();
    const journal = new FileManagerServiceJournal(path.join(root, "journal"));
    const record = journal.prepare({ id: "upgrade", action: "upgrade", desiredEnabled: true,
        spec: { ...previous, binPath: path.join(root, "new.js") }, upgrade: {
            previousSpec: previous, previousCandidateDigest: "a".repeat(64), candidateDigest: "b".repeat(64),
            snapshot: { platform, files: snapshot, initial: { enabled: true, processId: null, identity: null } },
        } });
    journal.save({ ...record, phase: "stopping" });
    journal.save({ ...record, phase: "writing" });
    return { host, root, previous, record: journal.read("upgrade"), files: getServiceFiles("user", host) };
}
it.each(["linux", "darwin"] as const)("%s 真实文件仅切换管理契约，保留业务字节，拒绝重复派发", platform => {
    ...
    for (const file of ["config.yaml", "auth.json", "id_map.db"])
        expect(fs.readFileSync(path.join(f.previous.workspace, file), "utf8")).toBe(`original:${file}\r\n`);
...
it.each(["definition", "metadata"] as const)("原 %s 被同字节替换也拒绝切换", key => {
    ...
    fs.writeFileSync(temporary, bytes, { mode: 0o600 }); fs.renameSync(temporary, f.files[key]);
