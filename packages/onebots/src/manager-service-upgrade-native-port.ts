...
export interface ManagerUpgradeNativeDependencies {
    ...
    /** Windows 测试 seam；生产使用 WindowsServicePlatform.reloadReplacing。 */
    replaceWindowsDefinition?(previousDefinition: Buffer, enabled: boolean): Promise<void>;
...
export function createManagerServiceUpgradeNativePort(
    ...
    let previousWindowsDefinition: Buffer | undefined;
    ...
    function record(input: ManagerServiceRecord, phase: ManagerServicePhase) {
        ...
        if (
            ...
            (host.platform === "win32"
                ? value.managerSpec.scope !== "system" || host.isElevated !== true
                : value.managerSpec.scope === "system" && host.uid !== 0)
        )
            throw failure();
        const identity = canonicalServiceJson({
            id: value.id,
            spec: value.managerSpec,
        ...
        return value as ManagerServiceRecord & {
            upgrade: NonNullable<ManagerServiceRecord["upgrade"]>;
        };
    }
    function filesUnchanged(value: ManagerServiceRecord, target: boolean) {
        const upgrade = value.upgrade!;
        const captured = captureManagerServiceRemoval(
            target ? value.managerSpec : upgrade.previousSpec,
            host,
        );
        try {
            if (
                !isDeepStrictEqual(captured.snapshot, target ? written : upgrade.snapshot.files) ||
    ...
    function candidate(value: ManagerServiceRecord, previous = false) {
        verifyManagerServiceCandidate(
            previous ? value.upgrade!.previousSpec : value.managerSpec,
    ...
    function maintenance(value: ManagerServiceRecord, released = false) {
        ...
        if (
            !marker ||
            marker.operationId !== value.id ||
            marker.candidateDigest !== value.upgrade!.candidateDigest ||
            (released ? marker.phase !== "released" : marker.phase !== undefined) ||
            readServiceMigrationPending(value.managerSpec.workspace)
        )
            throw failure();
        return marker;
    }
    async function quiet(value: ManagerServiceRecord, enabled: boolean) {
        ...
        if (
            ...
            (host.platform !== "win32" && !(await confirm(value.managerSpec.workspace))) ||
            !isDeepStrictEqual(state, await driver.inspect())
        )
            throw failure();
    }
    async function currentManager(
        value: ManagerServiceRecord,
        released: boolean,
    ): Promise<MigrationManagerState> {
        if (!driver) throw failure();
        candidate(value);
        ...
        if (
            ...
            before.enabled !== value.desiredEnabled ||
            ...
            manager.serviceMigration.pending !== !released ||
            ...
            (released && manager.gateway.actual !== manager.gateway.desired)
        )
            throw failure();
        return manager;
    }
    const port: ManagerServiceUpgradePort = {
        async verifyPrepared(input) {
            const value = record(input, "prepared"),
                spec = value.managerSpec;
            ...
            if (
                value.upgrade.snapshot.files.definition.path !== files.definition ||
                value.upgrade.snapshot.files.metadata.path !== files.metadata
            )
                throw failure();
            ...
            const before = await driver.inspect(),
                initial = value.upgrade.snapshot.initial;
            if (
                before.enabled !== initial.enabled ||
                before.processId !== initial.processId ||
                before.identity !== initial.identity ||
            ...
            if (before.running) {
                ...
                if (
                    ...
                    manager.manager.pid === process.pid ||
                ...
                !before.quiescent ||
                (host.platform !== "win32" && !(await confirm(spec.workspace)))
            )
                throw failure();
        ...
        async prepareMaintenance(input) {
            ...
            await prepareManagerUpgradeWorkspace(value.managerSpec.workspace, {
                ...
                candidateDigest: value.upgrade.candidateDigest,
            });
        },
        async writeCandidate(input) {
            ...
            if (host.platform === "win32") {
                const file = getServiceFiles(value.managerSpec.scope, host).definition;
                const current = new ConfigurationFile(file).readRaw();
                if (current.revision !== value.upgrade.snapshot.files.definition.sha256)
                    throw failure();
                previousWindowsDefinition = Buffer.from(current.bytes);
            }
        ...
        async restoreEnablement(input) {
            ...
            await quiet(value, false);
            if (host.platform === "win32") {
                if (!previousWindowsDefinition) throw failure();
                if (dependencies.replaceWindowsDefinition)
                    await dependencies.replaceWindowsDefinition(
                        previousWindowsDefinition,
                        value.desiredEnabled,
                    );
                else {
                    if (!(driver instanceof WindowsServicePlatform)) throw failure();
                    await driver.reloadReplacing(previousWindowsDefinition, value.desiredEnabled);
                }
            } else await driver!.reload(value.desiredEnabled);
        ...
        async verifyCandidate(input) {
            ...
            do {
                try {
                    candidateManagerId = (await currentManager(value, false)).manager.id;
                    return;
                } catch {
                    /* 只等待新实例就绪，不重发启动。 */
                }
        ...
        async releaseCandidate(input) {
            ...
            if (value.upgrade.snapshot.initial.processId === null) {
                filesUnchanged(value, true);
                await releaseStoppedManagerServiceUpgrade(value, host, driver);
                return;
            }
            if (!candidateManagerId) throw failure();
            await currentManager(value, false);
            maintenance(value);
            const socket =
                host.platform === "win32"
                    ? undefined
                    : inspectPrivateControlSocket(value.managerSpec.workspace);
            const result: unknown = await createLocalControlTransport(
                value.managerSpec.workspace,
            ).request("POST", "/api/control/service-upgrade/release", {
                operationId: value.id,
                candidateDigest: value.upgrade.candidateDigest,
                managerId: candidateManagerId,
            });
            if (
                !isDeepStrictEqual(result, { released: true }) ||
                (host.platform !== "win32" &&
                    inspectPrivateControlSocket(value.managerSpec.workspace) !== socket)
            )
                throw failure();
        },
        async verifyReleased(input) {
            ...
            if (!candidateManagerId || marker.managerId !== candidateManagerId || marker.completion)
                throw failure();
