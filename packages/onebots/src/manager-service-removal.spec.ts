...
import { captureManagerServiceRemoval, type ManagerServiceRemoval } from "./manager-service-removal.js";
...
function fixture(platform: "linux" | "darwin" = "linux") {
    ...
    roots.push(root);
    const host: ServiceHost = { platform, homedir: root, uid: process.getuid?.(), env: {},
        exec: vi.fn(() => { throw new Error("no OS"); }), spawn: vi.fn(async () => { throw new Error("no spawn"); }) };
    const spec: ManagerServiceSpec = { schemaVersion: 1, runtimeKind: "control", scope: "user",
        workspace: path.join(root, "data"), workingDirectory: root, nodePath: process.execPath,
        binPath: path.join(root, "bin.js"), host: "127.0.0.1", port: 6727 };
    ...
    return { root, host, spec, files, capture() {
        const plan = captureManagerServiceRemoval(spec, host); plans.push(plan); return plan;
    } };
}
describe("管理服务卸载文件端口", () => {
    it.each(["linux", "darwin"] as const)("%s 按阶段删除且保留业务目录", platform => {
        const f = fixture(platform), plan = f.capture();
        ...
        expect(fs.readFileSync(path.join(f.spec.workspace, "config.yaml"), "utf8")).toBe("broken: [");
    ...
    it.each(["definition", "metadata"] as const)("同字节替换 %s 拒绝删除任何文件", key => {
        const f = fixture(), plan = f.capture();
        const bytes = fs.readFileSync(f.files[key]), mode = fs.statSync(f.files[key]).mode & 0o777;
        fs.unlinkSync(f.files[key]); fs.writeFileSync(f.files[key], bytes, { mode });
    ...
    it("阶段间定义重新出现会拒绝删除元数据", () => {
        const f = fixture(), plan = f.capture();
        plan.removeDefinition(); fs.writeFileSync(f.files.definition, "foreign");
        expect(() => plan.removeMetadata()).toThrow();
        expect(fs.existsSync(f.files.metadata)).toBe(true);
    });
    it("删除后 fsync 失败保留未知状态，不能重试或删除下一文件", () => {
        const f = fixture(), plan = f.capture();
        vi.spyOn(fs, "fsyncSync").mockImplementation(() => { throw new Error("disk"); });
        expect(() => plan.removeDefinition()).toThrow();
        expect(plan.verifyRemaining()).toBe(false);
        expect(() => plan.removeDefinition()).toThrow();
        expect(() => plan.removeMetadata()).toThrow();
    ...
    it.each(["symlink", "mode", "hardlink", "metadata", "definition"])("捕获拒绝 %s", problem => {
        const f = fixture();
        if (problem === "symlink") { fs.unlinkSync(f.files.definition); fs.symlinkSync(f.files.metadata, f.files.definition); }
        ...
        if (problem === "metadata") fs.writeFileSync(f.files.metadata, JSON.stringify({ ...f.spec, port: 6728 }));
        if (problem === "definition") fs.appendFileSync(f.files.definition, "changed");
        expect(() => f.capture()).toThrow();
        expect(() => captureManagerServiceRemoval(f.spec, { ...f.host, platform: "win32" })).toThrow();
    });
    it("dispose 全部释放且 close 已完成后报错不重复关闭", () => {
        const f = fixture(), plan = f.capture();
        const close = fs.closeSync, descriptors: number[] = [];
        vi.spyOn(fs, "closeSync").mockImplementation(fd => {
            descriptors.push(fd); close(fd);
            if (descriptors.length === 1) throw new Error("unknown close");
        });
        expect(() => plan.dispose()).toThrow(); plan.dispose();
