...
const spec: ServiceSpec = {
    scope: "system", configPath: "C:\\One Bots\\data\\custom.yaml",
    nodePath: "C:\\Program Files\\nodejs\\node.exe", binPath: "C:\\One Bots\\lib\\bin.js",
    workingDirectory: "C:\\One Bots", adapters: ["mock"], protocols: ["onebot-v11"],
    applications: ["zhin"],
};
...
describe("历史 Windows 系统托管契约", () => {
    it("uses the old WinSW identity and bin-adjacent daemon, not the new manager definition", () => {
        ...
        expect(validateLegacyWindowsSystemXml(xml.replaceAll("\n", "\r\n"), spec, state, wrapper)).toBe(true);
    });
    it("reproduces the historical JS runner argv, working directory and Windows file URL", () => {
        ...
        expect(runner).toContain('process.argv = [process.execPath, entry, ...["--service-runtime","run","-c","C:\\\\One Bots\\\\data\\\\custom.yaml","-r","mock","-p","onebot-v11","-t","zhin"]];');
    ...
    it.each([
        ["onebotsgateway.exe", "onebots-gateway"], ["<argument>5</argument>", "<argument>6</argument>"],
        ["--scriptoptions=", "--scriptoptions=--unsafe"], ["wrapper.js", "other.js"],
        ["30sec", "1sec"], ["<logmode>rotate</logmode>", "<logmode>append</logmode>"],
        ["</service>", "<extra>true</extra></service>"], ["<service>", "<!DOCTYPE service><service>"],
        ["<name>onebots-gateway</name>", "<name>&unknown;</name>"],
    ])("rejects changed historical contract %s", (from, to) => {
        expect(validateLegacyWindowsSystemXml(xml.replace(from, to), spec, state, wrapper)).toBe(false);
    });
    it.each(["C:relative", "\\\\server\\share", "C:\\One Bots\\..\\other", "C:\\a:stream", "C:\\trailing."])(
    ...
    it("does not reinterpret a user task as a system service", () => {
        ...
        expect(validateLegacyWindowsSystemXml(xml, { ...spec, scope: "user" }, state, wrapper)).toBe(false);
    });
});
