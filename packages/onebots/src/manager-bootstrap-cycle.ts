...
export function selectManagerBootstrapCycle(scope: ServiceScope, host: ServiceHost): string {
    ...
    try {
        ...
        if (!stat.isDirectory() || stat.isSymbolicLink() || fs.realpathSync(home) !== home ||
            (stat.mode & 0o7777) !== 0o700 || (process.getuid && stat.uid !== process.getuid())) throw failure();
    } catch (error) {
    ...
    try {
        ...
        const intent = closedServiceObject(new ServiceOperationStorage(initial).read("intent.json"),
            ["schemaVersion", "id", "service", "planDigest"]);
        if (intent.schemaVersion !== 1 || typeof intent.id !== "string" || !/^[A-Za-z0-9_-]{1,128}$/.test(intent.id)) throw failure();
        id = intent.id;
    } catch (error) {
        if ((error as NodeJS.ErrnoException).code !== "ENOENT" || records.length || metadata.kind !== "missing") throw failure();
        return id;
    }
    ...
    for (;;) {
        ...
        if (!installation) {
            if (metadata.kind !== "missing" || records.some(record =>
                record.action === "install" && !visited.has(record.id))) throw failure();
            return id;
        }
        if (installation.action !== "install" || installation.managerSpec.scope !== scope ||
            installation.status !== "succeeded" || installation.recoveryRequired) throw failure();
        let spec = installation.managerSpec;
        const upgrades = new Set<string>();
        for (;;) {
            const next = records.filter(record => record.action === "upgrade" &&
                isDeepStrictEqual(record.upgrade?.previousSpec, spec));
            if (!next.length) break;
            if (next.length !== 1 || upgrades.has(next[0].id) || next[0].status !== "succeeded") throw failure();
            upgrades.add(next[0].id);
            spec = next[0].managerSpec;
        }
        const removed = records.filter(record => record.action === "uninstall" &&
            isDeepStrictEqual(record.managerSpec, spec));
        if (!removed.length) {
            if (metadata.kind !== "control" || !isDeepStrictEqual(metadata.spec, spec)) throw failure();
            return id;
        }
        if (removed.length !== 1 || removed[0].status !== "succeeded" ||
            (metadata.kind === "control" && isDeepStrictEqual(metadata.spec, spec))) throw failure();
