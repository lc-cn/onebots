...
function record(): ManagerServiceRecord {
    ...
    return {
        ...
        removal: {
            platform: "darwin",
            files: { definition: file, metadata: file },
...
describe("管理服务卸载 CLI", () => {
    it.each([false, true])("成功仅输出操作摘要和保留数据说明，system=%s", async system => {
        ...
        expect(result).toEqual({
            exitCode: 0,
            output: "操作 uninstall-test-id：succeeded（completed）\n系统托管已卸载，工作区、认证、配置、账号数据及日志全部保留。",
        });
        safe(result.output);
    });
    it.each(["running", "failed", "interrupted", "succeeded"] as const)(
        "%s 且待对账时非零退出，不声称卸载成功",
        async status => {
            vi.mocked(uninstallManagerService).mockResolvedValue({
                ...record(),
                status,
