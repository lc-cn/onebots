...
export function captureInstalledManagerCandidate(
    ...
    if (record.action !== "install" || !["releasing", "completed"].includes(record.phase) ||
        !["running", "interrupted", "succeeded"].includes(record.status) ||
        !["linux", "darwin"].includes(host.platform) ||
        (spec.scope === "system" && host.uid !== 0)) throw failure();
    ...
    const dispose = () => {
        ...
        const closeFiles = () => {
            const file = held.pop();
            if (!file) return;
            try { file.close(); } finally { closeFiles(); }
        };
    ...
    try {
        ...
        const verify = () => {
            ...
            const intent = closedServiceObject(intentFile.read(), ["schemaVersion", "id", "service", "planDigest"]);
            const binding = closedServiceObject(candidateFile.read(), ["schemaVersion", "id", "planDigest",
                "candidateId", "candidateDigest", "spec"]);
            if (intent.schemaVersion !== 1 || binding.schemaVersion !== 1 ||
                intent.id !== record.id || binding.id !== record.id ||
                typeof intent.planDigest !== "string" || !digestPattern.test(intent.planDigest) ||
                binding.planDigest !== intent.planDigest ||
                typeof binding.candidateId !== "string" ||
                typeof binding.candidateDigest !== "string" || !digestPattern.test(binding.candidateDigest))
                throw failure();
            const template = parseManagerServiceSpec(intent.service);
            const boundSpec = parseManagerServiceSpec(binding.spec);
            if (!isDeepStrictEqual(boundSpec, spec) ||
                !isDeepStrictEqual({ ...template, binPath: spec.binPath,
                    workingDirectory: spec.workingDirectory }, spec)) throw failure();
            const candidate = readVerifiedManagerCandidate(path.join(home, "versions"), binding.candidateId);
            if (candidate.operationId !== record.id || candidate.planDigest !== intent.planDigest ||
                candidate.directory !== path.join(home, "versions", binding.candidateId) ||
                spec.workingDirectory !== candidate.directory ||
                spec.binPath !== path.join(candidate.directory, "node_modules/onebots/lib/bin.js") ||
                managerCandidateDigest(candidate) !== binding.candidateDigest) throw failure();
            const actual = verifyManagerServiceCandidate(spec, binding.candidateDigest);
            if (actual.directory !== candidate.directory || actual.id !== candidate.id) throw failure();
        };
        verify();
...
function privateDirectory(directory: string): fs.Stats {
...
function captureBinding(file: string) {
    ...
    try {
        ...
        const read = (): unknown => {
            ...
            if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 ||
                stat.size > 32_768 || (stat.mode & 0o7777) !== 0o600 ||
                (process.getuid && stat.uid !== process.getuid()) ||
                stat.dev !== original.dev || stat.ino !== original.ino ||
                anchor.nlink !== 1 || anchor.size !== stat.size || anchor.mode !== stat.mode)
                throw failure();
            const buffer = Buffer.alloc(32_769);
            const count = fs.readSync(descriptor, buffer, 0, buffer.length, 0);
            const after = fs.fstatSync(descriptor);
            const current = fs.lstatSync(file);
            if (count !== stat.size || current.dev !== stat.dev || current.ino !== stat.ino ||
                current.nlink !== 1 || after.ctimeMs !== stat.ctimeMs ||
                after.mtimeMs !== stat.mtimeMs || after.size !== stat.size ||
                current.mode !== stat.mode) throw failure();
