...
`install` 会先按前台启动的真实路径动态加载当前 `-r` / `-p` 插件，确认插件注册了 CLI 名称对应的工厂与配置 Schema，并校验 `-c` 指向的完整配置；插件缺失、初始化失败、注册契约不完整或配置无效时不会写入服务定义。预检通过后，它会记录这些参数与运行路径并设置开机或登录自启，但不立即启动。`start` 和 `restart` 会根据保存的工作目录与插件列表重新预检当前环境；重启预检失败时不会先停止正在运行的实例。重复执行 `install` 会更新固定的 `onebots-gateway` 服务。`uninstall` 不会删除配置、数据库或日志。
...
`update` 修改依赖后会通过更新后的 CLI 子进程重新加载服务插件并校验配置；只有预检成功才更新服务定义并重启。重启后还会等待 `/health` 声明 `onebots` 应用身份与目标版本，并确认 `/ready` 可用或仍处于允许继续配置的首次部署状态；超时会保留最后一次探针证据并返回失败，不会把“重启命令已执行”误报为更新成功。预检失败时当前运行实例保持不变，但磁盘上的依赖已经更新，应根据错误修复或使用包管理器回退后重试。交互模式选择暂不重启时，命令会明确提示运行中的仍是旧实例，随后可执行 `onebots restart` 完成切换。
