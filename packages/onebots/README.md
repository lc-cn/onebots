...
`install` 会先按前台启动的真实路径动态加载当前 `-r` / `-p` 插件，确认插件注册了 CLI 名称对应的工厂与配置 Schema，并校验 `-c` 指向的完整配置。插件事务只能修改该 CLI 名称承诺的工厂、元数据与 Schema；额外注册其他适配器或协议、冒领导入前已有的注册身份、初始化失败、注册契约不完整或配置无效时都会回滚，并且不会写入服务定义。同一包重复加载保持幂等，协议的不同版本可以共享协议元数据并分别注册。预检通过后，它会记录这些参数与运行路径并设置开机或登录自启，但不立即启动。`start` 和 `restart` 会根据保存的工作目录与插件列表重新预检当前环境，并检查服务状态目录、`service.json`、平台定义及其父目录的权限；隔离运行时预检与实例取证完成后，CLI 会在调用进程管理器前重复这项检查和定义一致性验证，任一高风险漂移都会终止操作，重启不会先停止正在运行的实例。操作系统命令返回后，CLI 还会等待 `/health` 与 `/ready`，核对目标版本和进程 `instance_id`；需要切换进程时，只有不同的新实例实际接管端口才报告成功。重复执行 `install` 会更新固定的 `onebots-gateway` 服务。`uninstall` 不会删除配置、数据库或日志。
