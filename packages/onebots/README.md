...
`install` 会先按前台启动的真实路径动态加载当前 `-r` / `-p` 插件，并校验 `-c` 指向的完整配置；插件缺失、初始化失败或配置无效时不会写入服务定义。预检通过后，它会记录这些参数与运行路径并设置开机或登录自启，但不立即启动。重复执行会更新固定的 `onebots-gateway` 服务。`uninstall` 不会删除配置、数据库或日志。
