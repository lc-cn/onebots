...
- 12f6492: 首次安装与旧服务迁移保留随附 Web 工件，并将其冻结至私有工件快照，避免安装时意外依赖 npm 可用性或临时归档目录。
...
- 39a61cf: 运行工件同时提供当前 Web 包，安装计划冻结其版本与摘要并校验宿主原始依赖，修复未发布版本在本地及 CI 安装时尝试从 npm 下载 Web 的问题。
...
- 63b98ad: 本地扩展工件创建安装计划时从经过摘要校验的 tgz 读取包元数据，避免查询尚未发布的 npm 版本；保持精确版本和必需 peer 校验。
