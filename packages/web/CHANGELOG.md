...
- 9523c2c: 将账号与协议配置整理为平台账号、协议出口、运行设置和确认应用四个步骤，补充对象化编辑、协议作用域说明、语义字段分区、明确的下一动作与可访问键盘导航。
...
- 8f555e0: 优化概览指标与空状态、平台和框架选择卡片、诊断筛选与日志标签，以及设备会话列表和访问说明的信息层级。
...
- f3b03eb: 重做管理控制台的信息架构、品牌入口、运行概览与任务工作区，统一扩展、配置、诊断面板的产品层级、响应式布局与明暗主题；补齐可信首次安装判定、全局账号验证入口，以及服务迁移和进程所有权异常时的只读保护。
- 47c4e71: 移除已由设备配对控制台替代的旧Web登录页、路由及用户名密码和URL Token登录入口，补充管理端登录、退出与恢复指引。
...
- 4aab9c4: 提高控制台面板与侧栏透明度，为本地终端启用透明渲染和毛玻璃背景。
