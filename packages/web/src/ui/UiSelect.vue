...
import { computed, onUnmounted, ref, useAttrs, useId, watch } from "vue";
...
const activeOptionId = computed(() =>
    isOpen.value && highlightIndex.value >= 0
        ? `${listboxId}-option-${highlightIndex.value}`
        : undefined,
);
...
<template>
    <div ref="rootRef" class="relative" @focusout="onFocusout">
        <button
            ...
            :disabled="disabled"
            class="ui-select-trigger flex h-9 w-full items-center justify-between gap-2 rounded-control border border-border bg-surface px-3 text-sm transition-opacity focus-visible:border-accent focus-visible:shadow-[0_0_0_3px_var(--ring)] disabled:cursor-not-allowed disabled:opacity-50"
