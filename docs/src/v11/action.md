...
| 字段名 | 数据类型 | 默认值 | 说明 |
| ----- | ------- | ----- | --- |
| `message_type` | string | - | 消息类型，支持 `private`、`group` |
...
| `sub_type` 或 `type` | string | - | `add` 或 `invite`，请求类型（需和上报消息中的 `sub_type` 字段相符） |
...
| `good` | boolean | 状态符合预期，意味着各模块正常运行、功能正常 |
