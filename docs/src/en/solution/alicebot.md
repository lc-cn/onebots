...
- The `alicebot` extension adds framework compatibility only. It does not enable a transport or edit account configuration.

## Generate both configurations

```bash
onebots frameworks --framework alicebot --account <platform.account_id>
```

For separate hosts, add `--origin http://<onebots-host>:6727` and `--framework_origin http://<framework-host>:<port>`. The output contains the exact OneBots YAML, AliceBot configuration, endpoint, and checks.
...
Copy the generated “AliceBot configuration” into the framework project. Keep the protocol, endpoint, and token identical on both sides. In containers, use a reachable service name instead of `127.0.0.1` for another container.
