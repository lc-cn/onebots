...
- The `pepperbot` extension adds framework compatibility only. It does not enable a transport or edit account configuration.
...
For separate hosts, add `--origin http://<onebots-host>:6727`. The output contains the exact OneBots YAML, PepperBot configuration, endpoint, and checks.
...
Copy the generated “PepperBot configuration” into the framework project. Keep the protocol, endpoint, and token identical on both sides. In containers, use a reachable service name instead of `127.0.0.1` for another container.
