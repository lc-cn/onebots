...
The adapter acknowledges fully validated callbacks immediately, serializes `sync_msg` pagination per customer-service account in the background, and persists cursors atomically. Stop/restart cancels and generation-isolates stale lifecycle requests. Event identities come from the official nested `event` payload, while callback sync tokens and raw encrypted/decrypted XML never reach downstream protocols. API responses are validated at runtime, and one business listener cannot block cursor advancement. Native capabilities include media, links, locations, mini programs, menus, account and servicer management, session state, event-response messages, upgrade service, and statistics. Temporary-media upload does not require an `agent_id`.
