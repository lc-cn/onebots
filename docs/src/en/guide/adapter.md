...
In the Web console, **Extensions** shows the default capability summary and complete manifest on every loaded platform card. This lets users inspect actions, events, message segments, and transports before creating an account or entering credentials. A third-party adapter without a manifest is explicitly marked as unknown instead of being treated as fully supported. After creating an account, open **Capability overview** from **Bots** to inspect overrides caused by its token, permissions, or event subscriptions. Summary counts include native and emulated capabilities, while explicitly unsupported entries remain visible. Permission, scene, and context restrictions appear on each item. Both views consume the runtime manifest, so they describe the adapters loaded in the current deployment rather than a static platform catalog.
