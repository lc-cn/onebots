...
::: warning
The one-command installer targets the published stable version and has not been migrated to `codex/service-architecture`. For the architecture branch, build from source and run `onebots serve --data-dir <workspace>`. Do not use the legacy installer to update this branch.
:::
...
`onebots send --data-dir /path/to/onebots-data --account platform/account --target-type private TARGET MESSAGE` connects to the existing manager. It does not read legacy login credentials, start accounts, or enable an output protocol. Target IDs remain strings, including leading zeros, unless `--target-id-type number` is explicit. The CLI prints an operation ID before submission. On timeout, disconnect, or unknown outcome, query it with `onebots send --data-dir /path/to/onebots-data --operation-id ID --json`; do not send again. The manager persists a request digest before dispatching to the bound gateway snapshot and does not repeat the same operation ID. The record excludes message text and targets.

If dependencies live in a dedicated runtime directory through `ONEBOTS_EXTENSION_ROOT`, run `onebots install` with the same environment. The install command persists that extension root as the service working directory, so Web extension installation, isolated preflight, and plugin resolution after a managed restart all use the same location even when the command is launched from another shell directory. `onebots doctor` separately compares the canonical extension root with the installed service working directory and asks you to reinstall the service definition when an older definition or environment drift splits them.
...
Doctor verifies that every extension catalog **Configure** target matches its plugin identity and that the install allowlist, pinned package versions, and adapter capability snapshots form one closed set. Deployment scripts and CI can inspect the `extension-catalog` check in `--json` output; missing, orphaned, version-mismatched, or configuration-target entries are all reported with their reasons. `doctor --json`, `status --json`, and `capabilities --json` reserve stdout for the complete command duration. Plugin initialization messages written through `console.log` or ordinary stdout are redirected to stderr, TTY pending text is not rendered, and final stdout contains exactly one JSON document. An unexpected exception also goes only to stderr with a nonzero exit code instead of masquerading as JSON. Before an account is configured, `/ready` keeps the management surface reachable and reports `configured: false`; doctor presents this as a warning. `/ready` returns HTTP 503 when an account is offline, a protocol outlet fails to start, or any account has no protocol outlet configured.
...
An enabled extension also offers **Disable and restart**. Disabling removes the adapter or protocol only from the next startup selection under `plugins`; it keeps the dependency on disk so the extension can be enabled again quickly. The server runs the same isolated startup preflight against the candidate configuration first. If an account or protocol outlet still references the extension, the request returns a specific diagnostic and leaves the current configuration unchanged. A disable operation exposes a stable operation ID, start time, and the latest success or failure result. If the browser or reverse proxy disconnects during a long preflight, the page keeps following that operation, resumes the safe restart after success, or displays the redacted diagnostic retained by the server after failure. A temporary inventory read failure is retried for a bounded period instead of being treated as failure or causing another disable request. A supervised service then completes a verified restart. A foreground process stays online and explicitly asks for a manual restart to stop the extension already loaded in that process.

After disable and restart are complete, the card offers **Uninstall disk dependency**. OneBots invokes the package manager only after the startup configuration no longer selects the extension, the current process proves that it is no longer loaded, and the package identity and version on disk are verifiable. Uninstall does not rewrite configuration or require another restart. If the package manager exits nonzero or post-removal verification fails after changing the dependency, manifest, or lockfiles, OneBots restores the exact previous version and verifies the metadata again. Uninstall also exposes a stable operation ID and terminal result, so a disconnected browser follows the original transaction instead of repeating the removal.
