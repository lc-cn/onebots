...
Doctor also verifies that the `data` path beside the configuration file is a directory that the current process can read, write, and traverse. The default database, security audit, and management-terminal log cache all use this directory, so managed and foreground processes no longer scatter cache files according to their working directories. A colliding regular file, incorrect volume mount, or insufficient permissions fails the deployment gate before runtime storage initialization. A missing path remains a warning by default; only explicit `--fix` creates and verifies it, and an existing conflicting path is never replaced.

`onebots setup` and foreground startup use the same data-directory boundary. First-time initialization and `setup --force` validate or create that directory before writing or backing up configuration. A conflicting mount target or insufficient permission therefore preserves the current configuration, existing backup, and conflicting path instead of reporting failure after a partial configuration has already taken effect. Foreground startup also passes this check before generating a missing default configuration and management credential.
