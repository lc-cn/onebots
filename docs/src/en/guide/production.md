...
Doctor also verifies that the `data` path beside the configuration file is a directory that the current process can read, write, and traverse. A colliding regular file, incorrect volume mount, or insufficient permissions fails the deployment gate before database or audit-log initialization. A missing path remains a warning by default; only explicit `--fix` creates and verifies it, and an existing conflicting path is never replaced.
