...
Extension installation also acquires a cross-process `.onebots-package-mutation.lock` lease in the shared runtime directory. Multiple managed instances, reverse-proxy retries, or parallel management requests therefore cannot interleave changes to `package.json`, lockfiles, and `node_modules`. A later request receives HTTP 409 with the active extension, host, process ID, operation ID, and start time. A random ownership token prevents an expired holder from deleting a later operation's lease. When the same host can prove that the process exited, its lease is reclaimed through an atomic rename before the next installation. A different host or PID namespace cannot use its local process probe as false exit evidence. If a process ID was reused or a remote instance disappeared, the lease is reclaimed after 30 minutes, which exceeds the complete dependency-install, three-pass isolated-preflight, and failure-recovery timeout boundary. An unverifiable lease inside that protection window is never guessed away; the diagnostic asks the operator to wait or inspect the runtime directory.
