...
Windows user services validate both the Task Scheduler XML and the executed `onebots-runner.cmd`, including Node.js, the CLI entry, configuration and plugin selection, working directory, and log redirection. A missing or modified file fails `service-definition`. Reinstalling or running user-level `doctor --fix` atomically rebuilds both files and verifies them against the same rendering contract instead of treating the XML path alone as trusted startup evidence.

Windows system services are verified against the WinSW XML that `node-windows` actually installs in the `daemon/` directory beside the CLI entry, together with its companion `.exe`. Doctor checks the Node.js executable, wrapper, CLI entry, complete arguments, working directory, log directory, and restart policy, and diagnostics identify the real XML path. A system service therefore no longer fails permanently because of a placeholder path in the state directory, while any drift in the actual startup contract still requires an administrator to rerun `onebots install --system`.
