...
In `/health`, `application` and `version` identify the running `onebots` application package, while `core_version` identifies `@onebots/core` separately. Prometheus publishes the application through `onebots_info` and Core through `onebots_core_info`. The Web **System** page also displays both values so an upgraded dependency cannot be mistaken for the running application release.
