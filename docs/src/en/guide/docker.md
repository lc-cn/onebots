...
On first run, a `./data` directory and a default `config.yaml` are created. The default contains no platform accounts, so it cannot attempt external connections with empty credentials. Without environment authentication or file credentials, the container writes a random 256-bit `access_token` to that file without printing the token to container logs. Read it from `./data/config.yaml` to sign in. If the deployment cannot expose the mounted file, inject a high-entropy `ONEBOTS_ACCESS_TOKEN` Secret instead; it overrides the file token and is never written to configuration or logs. Add accounts and protocol access tokens in the Web console. **Save and apply** hot reloads account and protocol changes immediately; only host settings such as the port, path, and database require `docker compose restart`.
