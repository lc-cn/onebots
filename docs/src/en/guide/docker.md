...
2. In the Space repo, add these three files (copy from this repo and preserve the `scripts/` subdirectory):
...
`Dockerfile.hf` is based on the official image `ghcr.io/lc-cn/onebots:master` and only adds the HF port, entrypoint, and dependency-free restore downloader, so builds are fast and no GitHub Packages build secret is needed.
