...
The default extension root is `/data/extensions`. Set `ONEBOTS_EXTENSION_ROOT` only when another persistent directory is mounted, and use an absolute in-container path so installation and loading cannot diverge when the startup directory changes.
...
| `/data/extensions/` | Dependency manifest and packages installed by the Web extension center; the entrypoint starts here |
...
Startup restore gives each artifact at most 60 seconds. The compressed archive is limited to 15 MiB, while configuration and extension recovery files are each limited to 1 MiB. Downloads first enter a same-directory private `0600` temporary file and replace the target atomically only after the complete body stays within its limit. A truncated or oversized response, or a symlinked target directory, therefore preserves existing persistent state and follows the safe fallback instead of treating a partial configuration as bootable. The temporary data archive is removed before download and after extraction, so a process restart cannot restore a stale archive when the new download fails.

Both the pre-pack backup check and startup restore accept only regular files and directories under relative paths, with at most 10,000 entries and 128 MiB after expansion. A data tree that cannot satisfy the restore contract degrades to configuration and extension-catalog backup instead of uploading a “complete archive” that startup must later reject. Directory listing, type inspection, and isolated extraction each have a 30-second deadline. Restore content first enters a private `0700` staging directory under `/data`; OneBots then verifies the exact entry set, actual file types, hard links, and total size, preflights every destination type, and overlays files as `0600`. Any failed check removes staging without overwriting existing files.
