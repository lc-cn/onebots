...
错误消息会包含完整路径，例如 `qq.my_bot.appid`、`qq.my_bot.onebot.v11.use_http` 或 `qq.my_bot: 账号至少需要配置一个已加载的协议出口`。配置了未加载的适配器或协议也会直接阻止启动，避免账号被静默忽略。Web 管理端保存配置、setup、doctor 和热重载使用同一验证器，无效内容不会写入配置文件。
