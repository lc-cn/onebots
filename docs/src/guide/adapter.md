...
启动 Web 管理端后，「功能扩展」会在已加载的平台卡片中直接展示默认能力摘要和完整清单，用户可在创建账号、填写凭据前确认动作、事件、消息段和连接方式的边界。未声明清单的第三方适配器会明确标记为未知，不会被推断为全能力支持。创建账号后，还可在「机器人管理」点击「能力概览」，查看账号 token、权限和事件订阅造成的实际覆写。概览数字只统计原生和模拟能力，明确标记为不支持的项目仍会保留在列表中；权限、场景和上下文限制也会随条目展示。两处都消费当前进程实际加载的运行时清单，因此比静态平台列表更适合确认一次部署的能力边界。
...
插件导入与上述契约检查构成一次注册事务，并在进程内串行执行。如果模块初始化抛错，或完成后缺少承诺的工厂或 Schema，OneBots 会恢复导入前的全部适配器、协议、Schema 与协议版本元数据。失败插件不会留下半注册状态，也不会使随后加载的插件产生虚假名称冲突；修复插件后应重新启动进程，让 Node.js 重新导入模块。
