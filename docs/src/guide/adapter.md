...
不指定适配器时，命令展示随当前 OneBots 版本发布的完整目录快照；`--register` 可临时加载一个已安装适配器的权威清单。命令不连接平台，也不加载协议。JSON 报告包含包名、版本、真实入口、`status`、四类统计与完整清单，适合选型比较和 CI 留档。插件加载失败会保留在 `errors` 并返回退出码 `2`；适配器未注册默认能力清单或目录证据不可用时 `complete` 为 `false` 并返回退出码 `1`。账号权限、订阅配置产生的覆写只能在服务启动后通过 `/api/adapters` 或 Web 能力面板确认。
...
Web 的“功能扩展”可浏览完整安装目录；账号和协议配置表单使用当前活动运行代实际安装并注册的适配器、协议及 Schema。插件自己的注册 Schema 是运行时校验、表单分区、敏感字段与动态列表的唯一来源；主程序不维护第二份配置字段清单。
...
模块初始化完成后，加载器还会核对安装计划中的插件契约：适配器必须注册计划中同名的工厂与 Schema，协议必须注册对应的协议工厂与 `<name>.<version>` Schema。只导出代码、漏执行注册或注册了错误名称的包会立即加载失败，并在安装验证、doctor 和服务预检中指出缺少的注册项。
