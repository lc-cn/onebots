...
首次启动不会复制示例配置，也不会预注册一批平台和协议。没有 `config.yaml` 时，容器等待你完成向导；此时没有网关端口服务，健康检查也不会显示成功。配置表单根据你安装并选择的适配器、协议 Schema 展示，账号、Token 和协议出口由你填写并确认保存。取消或退出未保存时，不生成配置文件。已有配置保持不变。
