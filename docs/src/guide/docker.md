...
首次运行会在当前目录下创建 `./data`，并在其中生成默认 `config.yaml`。默认文件不包含平台账号，因此不会用空凭据连接外部平台；启动日志会显示自动生成的管理端初始凭据。登录 Web 管理端添加账号、设置协议访问令牌后，“保存并应用”会立即热重载账号与协议。只有端口、路径、数据库等宿主参数变更时，页面才会明确提示执行 `docker compose restart`。
