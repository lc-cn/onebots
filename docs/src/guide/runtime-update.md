...
顶层 `onebots update --check --data-dir <工作区>` 只检查网关运行版本，有更新退出 2，无更新退出 0；交互终端运行 `onebots update --data-dir <工作区>` 进入相同确认流程。旧 `--yes`、`--packages-only`、系统托管及插件选择参数已移除，不再原地改写程序包或重启系统服务。管理程序本身的升级事务尚未提供。
