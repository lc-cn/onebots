...
**脚本从公开 npm 下载程序，只有发布包含新管理服务的版本后才能完成安装。** 缺少新架构产物时会保留候选目录并明确失败，不运行旧 CLI。当前源码打包产物已经在 GitHub 托管的 Ubuntu 和 macOS runner 上分别通过真实 systemd 系统级、launchd 用户级生命周期验收；这项证据不等同于公开 npm 已发布。
...
系统托管维护管理服务；TUI/Web 的启停控制的是独立网关。当前 CI 已验证从安装包执行安装、启动、状态、重启、停止和卸载，并验证管理端在线、空白配置、进程实例换代及业务数据保留。机器重启、旧服务迁移故障恢复和管理程序原生升级仍需单独的平台验收。
