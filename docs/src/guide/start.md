...
::: warning
本文的一键安装脚本面向已发布的稳定版本，尚未迁移到 `codex/service-architecture`。重构分支请从源码构建后使用 `onebots serve --data-dir <工作区>` 启动管理服务；不要用旧脚本更新该分支。
:::

## 交互式安装与配置

首次使用推荐通过 [OneBots 工作台](/guide/tui) 选择适配器、填写安装凭据、选择协议和框架。统一确认后才安装扩展依赖；依赖验证通过后，继续在终端配置账号、启用协议和管理服务。
...
已安装 OneBots 时，在运行目录执行 `onebots ui -c config.yaml`。下面的管道安装命令适用于无需交互的自动部署。
...
`doctor` 会验证扩展目录中每个“去配置”入口与插件身份一致，并确认安装白名单、固定包版本和适配器能力快照形成闭合集合；`--json` 输出中的 `extension-catalog` 检查可供部署脚本和 CI 判定，缺失、孤立、版本错配或配置目标漂移时会列出全部目录项及原因。`doctor --json`、`status --json` 与 `capabilities --json` 在整个命令执行期独占 stdout：插件初始化通过 `console.log` 或普通 stdout 写入的消息会转到 stderr，TTY 等待提示不会渲染，最终 stdout 只包含一份 JSON 文档；意外异常也只进入 stderr 并使用非零退出码，不会把人类错误文本伪装成 JSON。首次尚未添加账号时 `/ready` 会保持管理面可访问，并返回 `configured: false`；doctor 会明确标为警告。账号离线、协议出口启动失败，或任一账号尚未配置协议出口时，`/ready` 返回 HTTP 503。
