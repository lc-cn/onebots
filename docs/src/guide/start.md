...
安装完成后，终端只需用于查看首次登录地址和鉴权码。后续可在 Web 管理端的“功能扩展”页面安装 Slack、Telegram 等平台适配器；安装完成后服务会自动重启，并通过分步说明引导准备平台凭据和填写账号配置。扩展中心会在下载依赖前校验当前配置；配置无效时不会开始安装。安装期间若配置被其他管理操作更新，扩展选择会合并到最新配置，避免覆盖并行修改。重启前还会使用守护服务的真实工作目录验证全部插件入口、注册契约和配置；预检失败时当前服务保持在线，页面会显示具体原因。
