...
Web 顶栏每 5 秒读取同一份 `/ready` 语义证据，区分「生产就绪」「待配置」「未就绪」和探针证据无效，而不再仅凭账号 online 数量显示绿色状态；点击状态可进入「系统信息」查看探针。系统页同时校验 `/health` 的应用身份、版本与实例标识，以及 `/ready` 自己的应用与实例身份、HTTP 状态、布尔结论、账号/协议计数和配置状态是否互相一致；两份响应的应用、版本或 `instance_id` 不同会显示“证据冲突”。因此代理返回的空 200、其他应用的响应、跨实例分流或矛盾 JSON 都不会被误报为可用。
