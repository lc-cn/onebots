...
核心停机编排会隔离扩展失败：一个协议停止失败不会阻止同账号的其他协议与账号监听器，一个账号失败不会阻止同适配器的其他账号，一个适配器或生命周期钩子失败也不会跳过其余适配器、数据库、WebSocket/HTTP、安全审计与 `close` 监听器。所有阶段完成后才汇总错误交给进程信号层，因此既保留可诊断的失败退出状态，也尽量避免强制退出前遗留资源。
