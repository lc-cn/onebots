...
Web 顶栏每 5 秒读取同一份 `/ready` 语义证据，区分「生产就绪」「待配置」「未就绪」和探针证据无效，而不再仅凭账号 online 数量显示绿色状态；点击状态可进入「系统信息」查看探针。系统页同时校验 `/health` 的应用身份、版本与实例标识，以及 `/ready` 自己的应用与实例身份、HTTP 状态、布尔结论、账号/协议计数和配置状态是否互相一致，因此代理返回的空 200、其他应用的响应或矛盾 JSON 不会被误报为可用。
...
服务启动后，`onebots status` 会同时显示进程管理器状态以及 `/health`、`/ready` 的语义结果，并在 readiness 证据中打印运行版本与 `instance_id`。输出会区分“运行中，已就绪”“运行中，待配置”“运行中，版本未验证”和“运行中，不可用”；在线主程序版本缺失或与当前 CLI 不一致、就绪响应无法归属到具体 OneBots 实例、服务已安装但未运行、任一探针失败时退出码为 `1`，服务未安装时为 `2`。首次未配置账号仍保持“待配置”且退出成功，便于继续使用管理面。因此 CI/CD 可以直接以 `onebots status` 作为轻量部署门禁，需要完整配置和插件诊断时再使用 `onebots doctor --json --strict`。
