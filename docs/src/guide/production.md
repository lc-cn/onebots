...
Web 顶栏每 5 秒读取同一份 `/ready` 语义证据，区分「生产就绪」「待配置」「未就绪」和探针证据无效，而不再仅凭账号 online 数量显示绿色状态；点击状态可进入「系统信息」查看探针。系统页同时校验 `/health` 的应用身份、版本与实例标识，以及 `/ready` 的 HTTP 状态、布尔结论、账号/协议计数和配置状态是否互相一致，因此代理返回的空 200 或矛盾 JSON 不会被误报为可用。
...
Web「系统信息」、扩展安装流程和管理终端的重启请求共用同一安全边界：先执行服务工作目录中的插件与配置预检，响应返回后调用 `app.stop()`，让账号、协议、WebSocket 与 HTTP 资源按生命周期顺序释放，再以守护进程可识别的失败码退出。优雅停机超过 30 秒会记录错误并交由守护进程强制切换，避免重启永久挂起。系统页和扩展页会记录操作前的 `instance_id`，只在 `/health` 返回应用身份为 `onebots` 且实例身份不同的新进程后刷新；仅有 HTTP 200、旧实例仍在线或身份字段缺失都不会被当作重启完成。
...
服务启动后，`onebots status` 会同时显示进程管理器状态以及 `/health`、`/ready` 的语义结果。输出会区分“运行中，已就绪”“运行中，待配置”“运行中，版本未验证”和“运行中，不可用”；在线主程序版本缺失或与当前 CLI 不一致、服务已安装但未运行、任一探针失败时退出码为 `1`，服务未安装时为 `2`。首次未配置账号仍保持“待配置”且退出成功，便于继续使用管理面。因此 CI/CD 可以直接以 `onebots status` 作为轻量部署门禁，需要完整配置和插件诊断时再使用 `onebots doctor --json --strict`。

`onebots update` 默认读取当前 `config.yaml` 的 `plugins` 选择，因此通过 Web 扩展中心后装的适配器和协议也会进入版本检查与更新；显式 `-r` / `-p` 仍可按类别覆盖配置，旧配置没有 `plugins` 时才回退到服务安装时的快照。包管理器成功退出后，更新器会从项目运行目录读取包清单；全局 CLI 场景则回退到当前 OneBots 安装根，逐个确认主程序与所有所选插件的实际版本等于查询到的目标版本。任一包缺失或偏离都会报告完整期望/实际清单，并在服务预检、定义改写和重启前终止。落盘版本全部可信后，才会从服务的 `workingDirectory` 启动更新后的 CLI 子进程，以同一插件列表执行无连接预检。只有新运行环境成功加载全部插件并通过配置校验，才会改写服务定义并按确认结果重启。重启后，更新命令会在有限重试窗口内同时探测 `/health` 与 `/ready`：在线进程必须声明 `onebots` 应用身份、本次目标版本和不同于重启前的有效 `instance_id`，且 readiness 成功或仍处于允许继续配置的首次部署状态，命令才会报告完成；超时错误保留最后一次健康与就绪证据，并建议检查 `onebots status` 和服务日志。交互模式选择暂不重启时，命令明确提示磁盘依赖已更新但运行中的仍是旧实例。预检失败时不会停止当前实例，也不会替换服务定义；由于依赖文件已经更新，现有进程可继续使用内存中的旧代码，但下次启动前必须根据错误修复依赖或使用 npm/pnpm 回退版本，然后重新执行预检或更新。
