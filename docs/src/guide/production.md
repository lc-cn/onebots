...
管理 API 发布账号能力前还会重新校验并复制冻结动态清单，再按结构内容与适配器默认清单比较。适配器即使为每个账号重新创建了内容相同的对象，也不会被误标为“账号专属清单”；真正不同的权限或订阅能力才进入 `accountCapabilities`。畸形清单和返回后被插件改写的对象不会成为 Web 能力证据。
