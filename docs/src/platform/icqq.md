...
- 文本、表情、图片、闪照、语音、视频、@、回复、分享、位置、互动、Markdown、合并转发、JSON 与 XML 消息
- 私聊、群聊与 QQ 频道撤回，历史、合并转发、已读、回应、精华消息和群公告
...
频道发送使用标准的独立寻址：`scene_id` 是 `channel_id`，同时传入 `guild_id`；频道用户提及通过字符串 `at.id` 传入 tiny ID。适配器返回的 opaque `message_id` 保存撤回所需的原生路由，调用方不需要也不应自行拼接 ID。平台扩展动作还提供频道链接分享、黑名单与好友分组、用户/群头像 URL、群文件容量/详情/跨群转发，以及离线文件查询和转发。
