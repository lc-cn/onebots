...
- 已审计核心所需的 17 个 OneBot 动作均有协议入口；第三方插件仍须单独核对。
- `zhenxun` 兼容扩展只补充框架兼容能力；它不会开启协议传输，也不会修改账号配置。

## 生成两端配置

先用真实账号键生成方案。输出会同时给出 OneBots YAML、真寻 端配置、端点和验证步骤：
...
安装或加载平台 Adapter、`onebot-v11` 协议和 `zhenxun` Application：
