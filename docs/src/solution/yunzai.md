...
- 已审计的 59 个直接动作中有 28 个 QQ 私有动作尚无通用实现。启用前先核对输出中的 `unsupportedActions`，涉及文件、群公告、频道或 QQ 资料的插件可能不可用。
- `yunzai` 兼容扩展只补充框架兼容能力；它不会开启协议传输，也不会修改账号配置。

## 生成两端配置

先用真实账号键生成方案。输出会同时给出 OneBots YAML、云崽 / TRSS-Yunzai 端配置、端点和验证步骤：
...
安装或加载平台 Adapter、`onebot-v11` 协议和 `yunzai` Application：
...
把生成结果中的“云崽 / TRSS-Yunzai 配置”复制到框架项目，并保持三项一致：
