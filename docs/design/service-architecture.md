...
本机管理程序升级新增本地产品工件入口：`onebots update --manager --artifacts <manifest>` 只接受产品构建生成的清单及同目录宿主/core归档，复核精确版本、发布目录结构和 SHA-256 后进入现有不可变候选与系统服务事务，不开放任意包名、URL 或 Web 自更新；依赖安装是否需要网络由工件内容决定。目标候选启动失败后，`onebots recover --operation <id> --rollback-upgrade` 只从持久升级阶段显式进入单向回退：核验并停止目标、按文件 CAS 恢复旧定义和 metadata、恢复原启用状态、关闭维护门禁，再按原意图启动旧 manager/gateway；重入逐文件接受精确 target 或 previous 字节并只补未恢复文件，第三方字节保持封锁。系统 reload/start 的结果无法由当前状态证明时不自动重派，即使维护标记已经释放，未完成 journal 仍阻止普通系统服务命令。真实 systemd 验收已改为先安装前一个 patch 身份，注入管理端口占用使当前 patch 启动失败并回退旧候选，再经同一公开 CLI 成功切换当前 patch；检查 manager、gateway 实例换代、运行代与配置摘要、网关意图、认证和用户数据保持，重启继续使用新候选，重复恢复不重启或重派。该脚本仍须对应远端 Linux job 通过后才能作为原生升级和故障回退证据。
