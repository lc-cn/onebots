...
公开前台入口只接受工作区与管理监听参数，不再用 `-c/-r/-p/-t` 直接启动旧网关。扩展选择与账号配置由管理端保存；旧系统服务先执行 `migrate`。迁移过渡期间，内部 `--service-runtime` 仍用于旧托管定义及回退，HF 的旧恢复安装链也尚待迁移；这些入口存在时，不宣称旧 App 管理鉴权已经删除。
