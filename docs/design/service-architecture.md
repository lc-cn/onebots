...
开发入口 `pnpm dev` 已统一为当前开发工作区的管理服务。网关共享的默认值合并、协议默认值注册与扩展加载独立于旧 App；五个协议使用 `registerProtocolDefaults`。旧 `App.registerGeneral` 暂以相同注册表承接旧消费者，不另建默认值副本。仍需迁移 `mcp` 的独立账号启动、`send` 的旧登录调用及 `update` 的旧服务重启，随后才能连同公开 App 构造入口移除整条旧管理链。
