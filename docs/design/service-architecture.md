...
状态：实施中，工作分支 `codex/service-architecture`。当前状态更新于 2026-09-10。整体验证 run `34413392887` 除 Windows 外全部通过；以下状态只按已完成 job 的证据分级，不表示完整产品已经交付。

| 阶段 | 当前状态 | 已有权威证据 | 收口前仍需完成 |
| --- | --- | --- | --- |
| 1. 控制契约与空白启动 | 主体已实现，最终验收中 | run `34413392887` 已通过安装后 Web/CLI/TUI、空白 Docker 管理端、设备配对、独立网关启停、跨入口恢复和容器重启后保留停止意图 | 按最终提交复跑，并补机器重启证据 |
| 2. 依赖版本与凭据流程 | 主体已实现，验收未闭合 | run `34413392887` 已通过当前宿主、Mock、协议工件和 ICQQ 私有必需 peer 的精确版本安装、授权拒绝、凭据清理、候选验证、显式激活和旧代保留 | 按最终工件复跑，并以真实平台账号验证安装后登录和运行 |
| 3. 配置与客户端统一 | 主体已实现，最终验收中 | run `34413392887` 已覆盖统一 `ControlClient` 的 Web/CLI/TUI、跨入口并发；动态 Schema、配置草稿、损坏配置修复、持久操作和控制日志均通过 | 按最终提交复跑并完成发布前审计 |
| 4. 系统托管与迁移 | Windows 收口中 | run `34413392887` 的 systemd 与 launchd 已通过真实生命周期、故障注入回退和 patch 升级；Windows SCM/管道/ACL 已实现 | 修复 Windows 首次管理候选验证，完成旧服务机器重启收据、实机生命周期和中断恢复验证 |
| 5. 产品验收与清理 | 最终清理中 | run `34413392887` 的受管协议回归覆盖根目录全部 21 个协议套件，12 个框架使用真实框架进程实跑；旧 `App`、旧服务控制器、旧运行入口和旧扩展安装器已删除 | 补真实平台账号证据、Windows 与最终全平台 CI，并完成发布前完整审计 |
...
下表是当前统一命令语义；平台是否可用于生产仍以对应最终 CI 和部署验收为准。
...
本节保留各检查点产生时的证据和限制，仅用于追溯，不能代表当前完成度。当前状态与剩余门槛只以上文开头的状态表为准；后续不再在本节追加进度文学，而是更新状态表和对应验收文档。
...
认证收敛约束已完成：Web 管理登录统一使用配对码、本地恢复码与设备会话；旧管理 `username/password`、根级管理 `access_token`、URL Token 登录和旧 App 管理入口已随旧运行宿主整体删除。迁移仍只清除旧配置根级管理凭据；浏览器会话、协议出口 `access_token` 和平台账号凭据继续保留，不因字段同名被移除。
...
旧认证与旧管理宿主已作为同一闭合运行岛退役：公开 App 构造、`--service-runtime` 执行分流、旧管理路由及其登录、WebSocket、SSE、终端、日志、备份和扩展管理辅助均已删除。对应能力由 manager、ControlAuth、GatewayApp 和统一客户端承接；旧服务回滚使用保存的旧版本工件，不依赖当前包重新开放旧入口。协议及账号 access_token、通用核心 TokenManager、网关环境清理和旧凭据迁移逻辑继续保留。

账号验证迁移发现首次扫码等待与网关 ready 之间的循环依赖，必须先拆分管理就绪与账号启动，并补齐停止取消边界。见[网关管理就绪与账号启动](./gateway-account-startup.md)。当前网关已收集带独立 ID 的有界挑战，旧完成不清除新挑战；管理服务已接入持久化提交、设备回执隔离及本地查询，CLI/TUI/Web 验证界面已接通，未知结果恢复仍待完成，不宣称真实平台登录已验证。

消息调试已迁移至独立网关与管理服务，并删除旧 App 缓存、调试路由和旧 Web 页面。GatewayApp 记录最多 300 条独立 JSON 快照，单条上限 16 KiB，复杂数据降级，不持有 HTTP 客户端。私有历史/清空 IPC 绑定管理实例、网关实例及请求标识，清空保留水位；失去确认不重放。管理端 `/api/control/message-debug/history`、`clear`、`stream` 使用设备会话；本地与 HTTP 共用客户端。SSE 每秒发送完整实例快照，最多 16 条流，撤销、网关不可用、关闭与写背压都会终止连接，轮询不重叠。

新 Web 面板通过共享客户端提供消息查看、方向/平台/账号/协议筛选和清空，默认不读取敏感消息；用户开启后每次读取完成等待一秒再刷新。授权或网关实例变化清除旧视图，清空确认丢失停止自动刷新并标记未核验。实际宿主已验设备撤销关闭流、本地与 HTTP 查询一致、停止网关保留管理及旧实例清空拒绝；真实 Mock/OneBot 双向消息分发不受坏调试数据影响。旧 App 的三个调试端点在有效旧管理凭据下均返回 404，其余未迁移路由仍保持鉴权。Web 当前使用历史轮询，尚未消费 SSE；这不代表其他旧管理入口或旧鉴权已退役。

一键安装脚本已移除旧 setup/update 原地流程：Linux/macOS 首次引导只向空目录安装管理程序，公开 npm 下载使用隔离环境和配置，验证新架构与 Web 工件后调用唯一服务 install/start/status。服务工作目录固定到持久 runtime；无默认协议、账号或长期登录凭据输出。成功重复执行不修改或重启；旧数据、失败候选及结果不明的操作不自动覆盖或重派。Windows 的 SCM 宿主、命名管道、ACL 与生命周期已实现，但最新实机 CI 尚未通过，PowerShell 首次引导暂时仍在修改前退出并引导 Docker。公开 npm 尚未发布本分支的新架构包；systemd 已取得真实生命周期、故障回退及 patch 升级证据，launchd、Windows 和最终整体验收仍在验证。
...
账号验证增加停止后的显式风险接受：与启停共用唯一生命周期队列，重新核验宿主开放、所有权、存储/配置健康、维护状态及网关完全停止后，同步追加接受回执。原 unknown、时间和设备归属不变，独立记录接受者摘要且不对外暴露；无自动停机、短信重发或验证码重提。CLI 要求 --accept-unknown，TUI 二次确认，Web 勾选风险并由后端权威核验。浏览器保留编号但服务端缺少原意图的恢复、旧验证入口删除及真实平台登录仍未完成。

缺失验证回执已提供封存恢复：停机事务内核对无原意图及无同编号待处理请求，持久记录独立封存回执，执行入口与落盘前均拒绝封存编号。CLI/TUI/Web 共用封存和只读查询，Web 不删除原编号，响应丢失或刷新后可恢复确认。该结果不冒充 SDK 未执行证明。已移除旧 Web VerificationPanel、useVerification、专属解析与重复类型；旧 App 后端验证仍依赖系统托管旧工件回退迁移后退役。

网关日志客户端已迁移至固定管理接口：CLI control logs、TUI 显式确认和 Web 手动读取共用严格响应契约，仅读取工作区 gateway.log 末尾64KiB。网关离线不影响读取，设备鉴权仍生效；读写共同拒绝链接、非私有权限和目录重定向，客户端移除终端控制序列。不提供PTY、任意路径、日志清空或自动读取。管理服务/安装操作日志及旧终端后端仍需继续迁移。

旧服务回退核查确认：现有备份只有服务定义、metadata与配置，尚无完整旧程序工件；回退恢复原bin路径可能执行被覆盖的新程序，不能据此删除新包的旧入口。后续须在覆盖旧安装之前保留完整自包含运行目录（主包、插件、生产依赖及内部pnpm链接布局），单独绑定原spec与指向旧工件的回退spec，继续使用原绝对configPath保留账号/ID数据。入口文件hash不构成完整闭包证明；外部链接、父级依赖、无法固定的Node运行依赖或第三方cwd资源需在停机前明确拒绝，不能静默搬迁。捕获副作用必须从当前只读capture步骤独立出来，并纳入持久操作与服务锁；此工件捕获及实际旧行为回退验收尚未实现。

完整目录快照基础能力已实现：逐文件有界复制和摘要校验，保留内部 pnpm 链接，将内部绝对链接转换为相对链接，源硬链接复制为独立文件；拒绝外部/损坏链接、特殊文件和危险权限。产物在私有目录中发布，失败候选保留待显式清理，不覆盖同编号产物。macOS 上 28 项专项通过，包含删除源目录后用当前 Node 运行 npm/pnpm 测试程序。该能力尚未接入迁移事务，也未固定 Node 或证明任意安装的依赖闭包；Windows 未实测，不能据此退役旧运行入口。下一步是绑定原服务契约、旧运行工件与独立回退契约，并完成事务回退验收。

回退文件层已区分原始备份、目标及恢复状态：只有服务定义和元数据允许派生旧工件路径，配置恢复原始字节；部分恢复和第三方改动均通过同一文件边界核对。Node 独立保留基础能力已通过本机 macOS 验证，删除临时源二进制后，副本仍可自检版本、架构、crypto 与 TLS。静态检查覆盖 Mach-O 所有切片，拒绝外部库、RPATH、加载器环境及未知命令；探测不继承 Node 或动态加载器环境。Linux/Windows 原生依赖检查明确未实现，插件原生依赖扫描、工件与迁移日志绑定及实际服务回退仍待完成，不能将本机 Node 验收视为完整运行闭包证明。

保留工件现已绑定迁移备份和回退端口：绑定时核对原目录及 Node 摘要，运行路径只能由原路径相对于源根目录派生；配置及账号数据目录必须独立于安装目录。停机、恢复文件及再次启动前重验工件，物理树中的 Mach-O/ELF 与原生扩展均接受静态依赖检查。macOS 整合测试中，新实例失败且原安装被删除后，持久迁移事务仍执行了保留 Node 下的旧程序，配置原始字节不变；OS 驱动为注入，非真实 launchd 验收。Linux 新增 ELF64 little-endian x64/arm64 与 glibc 系统库递归检查，官方 node:24 独立容器实测通过；Alpine/musl 和自定义加载路径仍拒绝。自动捕获必须先持久化意图，再绑定最终工件，目前尚未接入默认迁移入口，旧记录继续只读保留，不冒充已有工件。

捕获编排已接入协调器的 retain 步骤：先持久化 capturing-runtime 意图，再创建按操作 ID 定位的程序/Node 工件，随后生成新备份摘要并保留原始备份。绑定必须匹配原服务元数据；失败或响应未知保留原操作并封锁后续迁移，构造系统端口与停服均在绑定之后。单根混合目录现仅排除原配置、其 data 与 .control，程序链接若依赖被排除内容则拒绝；回退继续使用原绝对配置路径。整合测试通过真实服务参数运行保留程序，读取原配置与账号数据，确认数据未进入快照且字节未变；OS 驱动仍为注入。默认 migrate 入口暂未启用此步骤，全局主包与工作区插件的多根捕获仍待实现，不能宣称旧安装通用迁移完成。

Linux 检查已补充 musl 的系统配置与默认搜索顺序。独立、禁网且自动删除的官方 node:24-alpine 容器中，实际 Node 检查通过，外部搜索路径、符号链接配置、可写系统库均被拒绝。该证据只覆盖静态原生库检查，不代表 Alpine 内已运行完整服务迁移或回退。

多根工件捕获现复用原目录快照的复制、摘要与发布实现：在私有 runtime/fs 中保留所选目录的原路径关系，仅生成必要祖先，不遍历祖先的其他内容。跨根链接同时核验词法与真实目标边界，改写为镜像内相对链接；配置和数据排除规则仍生效。本机测试删除全局主包和工作区源目录后，副本中的主包与插件仍共享唯一核心实例；主包构建及迁移相关 21 个文件、216 项测试通过，2 项按宿主跳过。该能力目前接受明确选择的目录，自动发现 npm/pnpm 依赖闭包、绑定多根回退契约以及默认 migrate 接入仍未完成，不代表通用全局安装迁移已验收。

旧工件捕获编排现接入标准 npm/pnpm 目录发现：按主包及实际生产依赖、已安装 optional 和必需 peer 选择目录，pnpm 保留虚拟槽中的依赖别名；不遍历整个全局安装祖先，插件和外部应用拒绝不同宿主/核心身份。多根回退契约采用 schemaVersion 2，保留 v1 历史解析，绑定源目录集合、完整工件摘要及确定派生的 bin/cwd，配置仍使用原绝对路径。单目录、混合目录和全局目录均通过同一捕获编排及持久事务回退验证，22 个测试文件共 221 项通过、2 项按宿主跳过，主包构建通过。OS 驱动仍为注入；默认 migrate 入口、非标准动态资源支持与真实系统切换验收未完成。

默认 migrateInstalledService 已接入 retain：只读捕获后，先持久化工件捕获意图，发现并验证原程序/依赖/Node，绑定多根回退契约后才构造系统切换端口。入口专项采用本机真实工件和注入 OS 驱动，验证停止服务迁移后保持停止且收据可重验；依赖缺失时未产生 OS 变更，定义和配置原字节不变，重复迁移被未完成记录封锁。主包构建与迁移/CLI 回归 23 个文件共 227 项通过、2 项按宿主跳过。该入口仍使用 CLI 提供的目标管理程序，目标管理工件纳管、捕获早期失败的显式恢复与真实 systemd/launchd 切换尚待完成；不能据此宣称整个服务迁移与旧入口退役已完成。

迁移早期失败现可显式取消：`onebots recover --operation <id> --cancel-migration`（系统级加 `--system`）。仅接受 prepared/capturing-runtime 的中断记录，持服务锁重新捕获原定义、metadata、配置和稳定 OS 启停/启用状态；确认未变且无维护门禁后记为 cancelled/failed，rolledBack 保持 false。保留全部备份与捕获工件，不删除、不重启、不自动重试；重复取消只读返回，后续新迁移使用新操作 ID。已进入 stopping-old 或原配置变化则继续封锁。该路径不是停服后恢复或完整升级回滚。

上一轮 CI 暴露 pnpm 的 vitest 命令 shim 注入 NODE_PATH，使目录发现混入当前工具依赖；本机使用相同 pnpm 入口复现。解析现限定旧包所在位置及父级 node_modules，不沿用当前 CLI 注入的全局搜索目录。主包构建与 pnpm 入口下 25 个测试文件共 255 项通过、2 项按宿主跳过，覆盖默认迁移和取消后的新周期；远端 Linux CI 需由新提交重新验收。

默认迁移现准备独立管理候选：保留旧工件后先持久化 preparing-manager，复用 ManagerCandidateInstaller、可信工件冻结和双证明，使用 migration-<id> 的稳定安装操作与私有迁移绑定。重复准备只查询已有操作；目标只允许更换 bin/cwd，workspace、Node、监听与服务范围不能漂移。新备份绑定 targetCandidateDigest，原备份保留；重新取得工件仓库锁后核验候选，停旧、写目标、启动、验收和释放前均复核摘要。程序工作目录与业务数据工作区分离，配置和账号数据不搬迁。准备失败可沿用早期显式取消，保留候选历史，不重派下载或OS动作。

本机主包构建及 pnpm 入口下 26 个文件共 271 项通过、2 项按宿主跳过。候选准备专项使用注入安装器和真实私有文件/归档冻结，默认入口注入候选证明及OS驱动；尚非实际npm候选至系统服务切换验收。上一CI目录发现已通过，剩余默认迁移在捕获阶段失败；官方 node:24 Linux 容器完整Node捕获通过，入口夹具改用私有Node副本以排除工具缓存权限影响，远端原因与修复效果仍待新CI确认。迁移后的安装周期衔接、真实业务依赖启动、跨发布升级和实际OS切换仍需验收，旧入口尚不能据此全部退役。

真实 npm 产物验收已加入独立迁移链路：安装本仓主包/core归档后，经真实 pnpm 下载、依赖验证和管理启动验证取得候选双证明，再通过持久迁移事务切换服务定义及元数据。验证目标位于 manager-artifacts/versions、摘要与候选一致、原停止状态不启动、原配置和账号数据字节不变，重复准备只查询同一候选。与原有真实候选升级、首次安装/卸载/重装一起，本机 verify-manager-package.mjs 完整通过。旧运行程序为自包含测试夹具，OS 驱动注入；这不是生产账号迁移或实际 systemd/launchd 验收。下一步仍需衔接迁移来源的安装周期，才能完整支持迁移后的卸载重装。

迁移来源现接入统一安装周期：读取已成功迁移、原候选绑定和双证明作为真实安装起点，沿已完成卸载派生稳定的新安装 ID；不生成虚构的 initial-install，不改写原迁移记录。初始安装目录存在但意图缺失时仍拒绝，不能改用迁移来源绕过损坏历史。实际 npm 产物已串联迁移、卸载、自动重装及重复请求，验证原配置、账号数据和迁移记录保持不变；系统驱动仍为注入。

该串联发现并修复历史迁移备份被误判为孤立文件的问题：工件及候选绑定时在迁移记录中追加 previousBackupDigests，读取时验证历史备份摘要和格式；普通状态保存不能增删引用。只读恢复检查识别这些明确引用，未知备份仍阻止继续操作，不通过删除历史解除门禁。迁移与安装回归 26 个文件 272 项通过、2 项按宿主跳过；最后补充的历史引用及损坏初始绑定专项 3 个文件 14 项通过，真实 npm 产物串联通过。已推送的 5273d5b7 对应 CI 34316573837 成功；本批新增修改仍须对应提交的 CI 验收。实际原生系统托管、停服后的冷恢复、跨发布升级和旧入口清理仍未完成。
...
旧网关当前进程的 Application 查询与动态加载接口现已退役：`/api/applications`、`/api/applications/load`、其工厂入口以及其余旧 App/Web 管理路径均不再形成第二套扩展管理面；框架方案目录、连接计划和动态 provider 入口继续保留。Application 扩展由管理服务的安装计划写入不可变运行代，并在网关启动时注册和激活。当前包不再导出 `createOnebots`，公开 CLI 明确拒绝 `--service-runtime`；迁移与回退只读取历史定义和保存的旧版本工件。
...
真实 legacy 服务验收已扩展到 Ubuntu systemd 和 macOS launchd：先启动固定旧服务，通过公开 CLI 识别 `migration-required` 并仅执行一次迁移，再强杀 manager/服务进程树；后续只读轮询必须由 supervisor 拉起新 manager 与 gateway，保持 running 意图、用户数据、迁移 operation ID 和 journal 原字节。Windows 同批建立 SCM 底层边界和 x64/arm64 原生宿主：严格解析单行 JSON/CRLF，分离原生宿主与 Node manager PID，绑定 SCM `PathName`，提供注册、状态、启停和安全注销，并把两个 `.exe` 纳入 npm 工件及 CI 构建。后续已补齐 Windows 工作区权限、命名管道 ACL 和顶层生命周期；该检查点的“CLI 尚未开放”已经失效。最新 Windows 实机 CI 仍未通过，当前不能将实现存在写成生产验收完成。

本机管理程序升级新增本地产品工件入口：`onebots update --manager --artifacts <manifest>` 只接受产品构建生成的清单及同目录宿主/core归档，复核精确版本、发布目录结构和 SHA-256 后进入现有不可变候选与系统服务事务，不开放任意包名、URL 或 Web 自更新；依赖安装是否需要网络由工件内容决定。目标候选启动失败后，`onebots recover --operation <id> --rollback-upgrade` 只从持久升级阶段显式进入单向回退：核验并停止目标、按文件 CAS 恢复旧定义和 metadata、恢复原启用状态、关闭维护门禁，再按原意图启动旧 manager/gateway；重入逐文件接受精确 target 或 previous 字节并只补未恢复文件，第三方字节保持封锁。系统 reload/start 的结果无法由当前状态证明时不自动重派，即使维护标记已经释放，未完成 journal 仍阻止普通系统服务命令。真实 systemd 与 launchd 验收均先安装前一个 patch 身份，在旧 manager 退出后用控制 socket 障碍使当前 patch 启动失败并显式回退，再经同一公开 CLI 成功切换当前 patch；检查 manager、gateway 实例换代、运行代与配置摘要、网关意图、认证和用户数据保持，重启继续使用新候选，重复恢复不重启或重派。launchd 还以 PID、进程组和启动时间绑定重复恢复前后的实例。两套脚本仍须对应远端原生 job 通过后才能作为升级和故障回退证据。
