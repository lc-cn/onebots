...
| 2. 依赖版本与凭据流程 | 主体已实现，移除流程未闭合 | run `34413392887` 已通过当前宿主、Mock、协议工件和 ICQQ 私有必需 peer 的精确版本安装、授权拒绝、凭据清理、候选验证、显式激活和旧代保留 | 通过新运行版本完成扩展移除，按最终工件复跑，并以真实平台账号验证安装后登录和运行 |
...
下表定义公开命令的最终产品语义；尚未闭合的实现与验收以上文状态表为准。
...
| 4. 系统托管与迁移 | systemd、launchd、Windows、Docker 托管和旧安装迁移 | 启停语义一致；机器重启保留意图；迁移不双开账号；失败可按持久证据恢复 |
...
- 当前权威整体验证为 run `34413392887`：除 Windows 外的 job 已通过。Windows 真实生命周期仍未通过，真实平台账号安装后登录和运行也没有证据，因此本架构仍处于实施中。
- 只有顶部状态表的未完成项全部关闭、最终提交完成全平台验收且旧双轨实现清理完毕后，才能将本文状态改为完成并进入 `master`。
