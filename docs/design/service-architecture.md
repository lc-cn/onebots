...
捕获编排已接入协调器的 retain 步骤：先持久化 capturing-runtime 意图，再创建按操作 ID 定位的程序/Node 工件，随后生成新备份摘要并保留原始备份。绑定必须匹配原服务元数据；失败或响应未知保留原操作并封锁后续迁移，构造系统端口与停服均在绑定之后。真实副本回退测试已改由该编排自动捕获完成，仍使用注入 OS 驱动。默认 migrate 入口暂未启用此步骤：混合安装/数据目录和全局安装布局的拆分仍待实现，不能将有限的独立目录支持宣称为旧安装通用迁移完成。
