...
状态：实施中，工作分支 `codex/service-architecture`。当前状态更新于 2026-09-10。最新整体验证 run `34407671271` 尚未通过，以下状态只按已完成 job 的证据分级，不表示完整产品已经交付。

| 阶段 | 当前状态 | 已有权威证据 | 收口前仍需完成 |
| --- | --- | --- | --- |
| 1. 控制契约与空白启动 | 主体已实现，最终 CI 验证中 | 安装后 Web/CLI/TUI、空白 Docker 管理端、设备配对、独立网关启停和容器重启后保留停止意图已有实际验收 | 按最终提交复跑整体验收，并补机器重启证据 |
| 2. 依赖版本与凭据流程 | 主体已实现，验收未闭合 | 当前宿主、Mock、协议工件和 ICQQ 私有必需 peer 的精确版本安装、授权拒绝、凭据清理、候选验证、显式激活和旧代保留已有自动化证据 | 按最终工件复跑，并以真实平台账号验证安装后登录和运行 |
| 3. 配置与客户端统一 | 主体已实现，验收未闭合 | 安装后 Web/CLI/TUI 与跨入口并发验收已覆盖统一 `ControlClient`；动态 Schema、配置草稿、损坏配置修复、持久操作和控制日志已有回归 | 按最终提交复跑三入口、并发/中断恢复和完整审计 |
| 4. 系统托管与迁移 | 平台验证中 | systemd 的真实生命周期、故障注入回退和 patch 升级已在 run `34407671271` 通过；launchd 与 Windows 的 SCM/管道/ACL、生命周期及升级链路均已有实现 | launchd 与 Windows 在同一 run 的实机任务未通过；机器重启及完整中断恢复仍需验证 |
| 5. 产品验收与清理 | 部分完成 | 当前受管协议回归覆盖根目录全部 21 个协议套件；12 个框架已使用真实框架进程实跑；旧 `App`、旧服务控制器和旧运行入口已删除 | 补其余方案的对应等级证据、真实平台账号、最终全平台 CI 与发布前完整审计 |
