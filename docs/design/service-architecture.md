...
账号验证迁移发现首次扫码等待与网关 ready 之间的循环依赖，必须先拆分管理就绪与账号启动，并补齐停止取消边界。见[网关管理就绪与账号启动](./gateway-account-startup.md)。当前网关已收集带独立 ID 的有界挑战，旧完成不清除新挑战；管理服务已接入持久化提交、设备回执隔离及本地查询，CLI/TUI/Web 验证界面和未知结果恢复仍待完成，不宣称真实平台登录已验证。
