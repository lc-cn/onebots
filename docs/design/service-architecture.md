...
发送检查点本地验收：全量构建通过，766 个测试文件、5,488 项测试通过，1 项跳过；41 个发布包、2,511 个生产文件通过 tarball 边界检查。真实无协议 Mock 网关、CLI 发送/查询、跨管理服务重启去重及凭据撤销后的只读恢复已验证。Docker 安装脚本已补入发送及停机后回执查询，等待对应提交 CI；未验证真实平台在线发送，不据此宣称完整架构已完成。
